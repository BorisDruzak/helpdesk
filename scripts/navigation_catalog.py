...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="React/Vite `webapp`, typed `/api/web/*` contracts, role-aware `/app/*` routes, server-driven `default_workspace`/`available_workspaces`, operational legacy-shell cutover guardrails, bundle serving from aiohttp, shared realtime bridge over `/api/web/realtime/bootstrap` -> `/ws_ui`, support Playwright signoff and typed admin slices for inventory, device update actions, modules registry/actions, request-form builder, observer quick summary and trace drilldown surfaces.",
        aliases=(
            "webapp",
            "react app",
            "vite",
        ...
        checks=(
            ...
            "pnpm --dir webapp run check:remote -- --base-url http://192.168.100.17:8666",
            "pnpm --dir webapp run check:remote:webapp -- --base-url http://192.168.100.17:8666",
            "python -m pytest server/tests/test_web_session_api.py server/tests/test_web_support_api.py server/tests/test_web_admin_api.py server/tests/test_web_realtime_api.py server/tests/test_static_pages_handlers.py -v --tb=short",
        ),
        plan_required=True,
        docs_to_update=(
            repo_path(QUICK_LOOKUP_PATH),
