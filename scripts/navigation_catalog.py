...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="Launcher builds, self-update, upload, canary rollout, recommended-version behavior, React `/app/admin/agent-updates` build registry and rollout policy UI over web-session aliases `/api/web/admin/agent-builds*` and `/api/web/admin/agent-updates/rollout-policy`, Windows stable release bump to 3.1.39 for deeper focused-window CPU reduction on Windows/RDP/Proxmox graphics stacks after 3.1.38 launcher version retention and soft-shadow opt-in, 3.1.37 idle CPU/WMI reduction, 3.1.36 Remote Assist WebRTC timeout/capture cleanup, 3.1.35 SendInput/Linux interactive-control testing, 3.1.34 WebRTC packaging and startup-failure reporting and 3.1.33 Remote Assist runtime fixes, 3.1.32 requester diagnostic-result timeline wording, 3.1.31 requester timeline projection support, 3.1.30 GUI taskbar-window fix and 3.1.29 requester confirmation, diagnostic consent, diagnostics/deadline/assignment summaries and Russian event text GUI fixes, single-flight pending markers, truthful scheduled command results, launcher rollback after immediate crash, agent telemetry evidence for update flows and end-to-end update tracing.",
        aliases=(
            "agent update",
            "agent updates",
            "self update",
