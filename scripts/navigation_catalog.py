...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="React/Vite `webapp`, lazy route chunks, typed `/api/web/*` contracts, role-aware `/app/*` routes, public requester `/app/help` and `/app/ticket/*`, server-driven `default_workspace`/`available_workspaces`, operational legacy-shell cutover guardrails, bundle serving from aiohttp, shared realtime bridge over `/api/web/realtime/bootstrap` -> `/ws_ui`, real-data support/admin/reports/settings surfaces, typed notifications route/tab, typed module workbench and tech-alert aliases, honest knowledge placeholder, and typed admin slices for inventory, registry objects, device update actions, modules registry/actions, request-form builder, plus a full observer workbench with quick/traces/signatures/degradations/runtime tabs, global mode, trace detail with compact agent actions, and explicit detail/bundle error states.",
        aliases=(
            "webapp",
            "react app",
            "vite",
    ...
    Topic(
        ...
        summary="Always-on runtime lifecycle, tray behavior, auth-block system notifications, runtime diagnostics/logging, agent_observer_batch telemetry upload from action_trace, sticky local update request state (`requesting` / `requested` / `pending_restart`) and shutdown tracing.",
        aliases=(
            "always-on",
            "always on",
            "tray",
