...
TOPICS: tuple[Topic, ...] = (
    Topic(
        ...
        summary="WS handshake, reconnect-safe runtime ownership, state-level command waiters registered before dispatch wakeup, envelope V3, terminal-only outbox dedupe for retryable NACK safety, ACK/NACK, optional outbox batching, command_result semantics, command lifecycle DB import isolation and graceful sender/teardown behavior in server and agent runtimes.",
        aliases=(
            "protocol",
            "protocol v3",
            "ws_ticket_v3",
