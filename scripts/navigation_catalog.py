...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="Trace overlay, bounded action rows, agent telemetry, playbook and runtime-audit roots, and web-cabinet execution identity from durable `idempotency_key`/`operation_id`, server-generated `server_request_id`, then optional client request/correlation ids. Direct requester traces cover profile/preview/create, diagnostic-target outcomes, form-runtime preview/create, account-session, requester chat/closure, support chat/status/assignment, device linking, and registry binding. The local Knowledge runtime is retired: requester cabinet Knowledge events are not produced; only redacted historical evidence remains read-only. OBS1 integrity keeps bounded windows, per-check run reports, degradation queries, scan-observation/recurrence counters, Customer History and on-behalf historical-audience findings, trace bundles, typed admin detail APIs, support ticket trace cards, and the `/app/admin/observer` workbench.",
        aliases=(
            "observer",
            "trace overlay",
            "spans",
