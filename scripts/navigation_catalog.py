...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="Module install, desired state, reconcile, manifest, module registry, module_reconcile observer audit rows, mandatory observer SDK instrumentation, and the diagnostic capability foundation (`execution`/`deployment`/`safety`/`readiness`/`evidence`/`artifacts`, ticket-scoped readiness with stable reason codes/actions, provider projection, execution router envelopes, server_builtin operation records, persisted provider config, web-admin config aliases and non-agent provider routes).",
        aliases=(
            "module",
            "modules",
            "reconcile",
