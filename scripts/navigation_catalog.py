...
TOPICS: tuple[Topic, ...] = (
    ...
    Topic(
        ...
        summary="Ticket lifecycle, workflow profiles with executable per-transition role/required-field/comment/approval/evidence gates plus notification and SLA action markers, deterministic priority/SLA/OLA, second-precision business-calendar SLA due dates, standalone SLA JSON target calculation, typed request-template catalog in web settings, standalone versioned helpdesk policy/request-template registry with direct policy publication plus diff/deactivate/rollback-as-new-version API, effective registry policy resolution during ticket creation plus approval/closure/notification/visibility/reporting lifecycle reads, visual request-template constructor in the forms builder, executable request-template routing/approval/closure/visibility/notification/reporting policies, external notification channel provider/audit events, backend smart views for support queue slices plus active published custom smart-view filters, diagnostic policy evidence materialization for resolution passports, reporting-policy passport sections/evidence packages/export tags, chat, public access, queue behavior, request-template process context, request_template_key-aware ticket creation for local agent/public intake, legacy form-pack ticket_type inference, form-aware routing over ticket/request-form context, template default queue fallback, editable server-driven priority question fields/roles with old pack backfill that preserves custom policy field keys, P0 process priority support in OLA targets, 64-char ticket_type/request_kind slugs, and canonical ticket-root observer trace.",
        aliases=(
            "ticket",
            "tickets",
            "chat",
