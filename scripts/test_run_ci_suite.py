...
def test_main_runs_webapp_bundle_step_before_layered_pytests(tmp_path, monkeypatch):
    ...
    assert idle_by_step["active_risk_audit"] == run_ci_suite.DEFAULT_IDLE_TIMEOUT_SECONDS
    assert idle_by_step["observer_contamination_audit"] == run_ci_suite.DEFAULT_IDLE_TIMEOUT_SECONDS
    ...
    assert command_by_step["active_risk_audit"] == [
        sys.executable,
        str(tmp_path / "scripts" / "audit_active_risks.py"),
        "--workspace",
        str(tmp_path),
        "--strict",
    ]
    assert command_by_step["observer_contamination_audit"] == [
        sys.executable,
        str(tmp_path / "scripts" / "audit_observer_contamination.py"),
        "--workspace",
        str(tmp_path),
        "--strict",
    ]
    ...
    assert env_by_step["active_risk_audit"] is None
    assert env_by_step["observer_contamination_audit"] is None
