...
@pytest.mark.asyncio
async def test_web_knowledge_suggest_writes_requester_observer_event(test_client, test_engine) -> None:
    session_maker = async_sessionmaker(test_engine, expire_on_commit=False)
    suffix = uuid.uuid4().hex[:8]
    login = f"observer-knowledge-{suffix}@example.test"
    request_id = f"phase-d-knowledge-suggest-{suffix}"
    raw_query = f"VPN observer marker {suffix}"
    ...
    async with session_maker() as session:
        ...
        await repo.upsert_space(
            {
                "code": f"observer-knowledge-{suffix}",
                "title": "Observer Knowledge",
        ...
        item = await repo.create_item_draft(
            {
                "space_code": f"observer-knowledge-{suffix}",
                "slug": f"observer-knowledge-vpn-{suffix}",
                "item_type": "article",
                "title": f"Observer Knowledge VPN {suffix}",
                "summary": "Requester-safe observer knowledge suggestion",
                "visibility": "requester",
    ...
    assert trace.attrs_json["event_type"] == "knowledge_suggest_succeeded"
    assert trace.attrs_json["person_id"] == person.person_id
    assert span.attrs_json["route"] == "/api/knowledge/suggest"
    assert span.attrs_json["method"] == "POST"
    assert span.attrs_json["payload"]["surface"] == "requester_portal"
    assert span.attrs_json["payload"]["suggestion_count"] >= 1
    serialized = json.dumps({"trace": trace.attrs_json, "span": span.attrs_json}, sort_keys=True)
    assert raw_query not in serialized
    assert login not in serialized
    assert "raw-knowledge-token" not in serialized


@pytest.mark.asyncio
async def test_web_knowledge_ask_writes_requester_observer_event(test_client, test_engine) -> None:
    session_maker = async_sessionmaker(test_engine, expire_on_commit=False)
    suffix = uuid.uuid4().hex[:8]
    login = f"observer-knowledge-ask-{suffix}@example.test"
    request_id = f"phase-d-knowledge-ask-{suffix}"
    raw_query = f"VPN ask observer marker {suffix}"
    ...
    async with session_maker() as session:
        ...
        await repo.upsert_space(
            {
                "code": f"observer-knowledge-ask-{suffix}",
                "title": "Observer Knowledge Ask",
                "visibility": "requester",
                "lifecycle_status": "active",
                "allow_rag": True,
            },
        ...
        item = await repo.create_item_draft(
            {
                "space_code": f"observer-knowledge-ask-{suffix}",
                "slug": f"observer-knowledge-ask-vpn-{suffix}",
                "item_type": "article",
                "title": f"Observer Knowledge Ask VPN {suffix}",
                "summary": "Requester-safe observer knowledge ask",
                "visibility": "requester",
                "owner_actor_id": "owner",
                "reviewer_actor_id": "reviewer",
            },
        ...
        version = await repo.create_version(
            item["item_id"],
            {
                "title": f"Observer Knowledge Ask VPN {suffix}",
                "body_format": "markdown",
                "body": f"Fallback answer evidence for {raw_query}",
            },
            actor_id="support",
        )
        await repo.publish_item(item["item_id"], version["version_id"], actor_id="admin")
    ...
    assert trace.status == "succeeded"
    assert trace.attrs_json["source"] == "requester_knowledge"
    assert trace.attrs_json["event_type"] == "knowledge_ask_succeeded"
    assert trace.attrs_json["person_id"] == person.person_id
    assert span.attrs_json["route"] == "/api/knowledge/ask"
    assert span.attrs_json["method"] == "POST"
    assert span.attrs_json["payload"]["surface"] == "requester_portal"
    assert span.attrs_json["payload"]["answer_status"] == "ai_disabled"
    assert span.attrs_json["payload"]["retrieval_result_count"] >= 1
    serialized = json.dumps({"trace": trace.attrs_json, "span": span.attrs_json}, sort_keys=True)
    assert raw_query not in serialized
    assert login not in serialized
    assert "raw-ask-token" not in serialized
