...
@pytest.mark.asyncio
@pytest.mark.no_db
async def test_resolve_recommended_build_prefers_assigned_rollout():
    ...
    with patch("agents.agent_builds_handlers.AgentBuildsRepo", FakeAgentBuildsRepo), \
        ...
        build, source, assignment = await _resolve_recommended_build(object(), target="windows_amd64")
...
@pytest.mark.asyncio
@pytest.mark.no_db
async def test_resolve_requested_build_uses_assigned_rollout_when_version_is_omitted():
    ...
    with patch("agents.agent_builds_handlers.AgentBuildsRepo", FakeAgentBuildsRepo), \
         patch("agents.agent_builds_handlers.AgentRolloutRepo", FakeAgentRolloutRepo):
        build, source = await _resolve_requested_build(
            object(),
