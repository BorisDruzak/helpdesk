...
def _binding_access_error_response(payload: dict, *, status: int = 403) -> web.Response:
    return web.json_response(
        {
            "status": "error",
            "error": "binding_access_denied",
            "error_code": payload.get("error_code") or "BINDING_ACCESS_DENIED",
            "details": payload,
        },
        status=status,
    )
...
async def _require_agent_ticket_account_access(
    *,
    ...
    if not validation.get("valid"):
        return _binding_access_error_response(validation)
    allowed = (
    ...
    if not allowed:
        return _binding_access_error_response({"error_code": "BINDING_ACCESS_DENIED"})
    return None
