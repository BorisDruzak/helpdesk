...
from app.db.models import KnowledgeIngestionJob, KnowledgeNode
...
async def handle_web_knowledge_items(request: web.Request) -> web.Response:
    ...
    if request.method == "POST":
        ...
        async with get_session() as session:
            repo = KnowledgeRepo(session)
            item = await repo.create_item_draft(payload, actor_id=actor_id)
            bindings = []
            for binding in payload.get("bindings") or []:
                if isinstance(binding, dict):
                    bindings.append(await repo.add_binding(item["item_id"], binding, actor_id=actor_id))
            await session.commit()
        return web.json_response({"status": "ok", "item": item, "bindings": bindings})
    async with get_session() as session:
        items = await KnowledgeRepo(session).list_items(include_archived=True)
...
async def handle_web_knowledge_item_detail(request: web.Request) -> web.Response:
    ...
    try:
        async with get_session() as session:
            item = await KnowledgeRepo(session).get_item(item_id)
            bindings = await KnowledgeRepo(session).list_bindings(item_id)
        return web.json_response({"status": "ok", "item": item, "bindings": bindings})
...
async def handle_web_knowledge_item_versions(request: web.Request) -> web.Response:
    ...
    try:
        payload = await _json_payload(request)
        async with get_session() as session:
            version = await KnowledgeRepo(session).create_version(item_id, payload, actor_id=actor_id)
            await session.commit()
...
async def handle_web_knowledge_item_publish(request: web.Request) -> web.Response:
    ...
    try:
        async with get_session() as session:
            item = await KnowledgeRepo(session).publish_item(item_id, payload.get("version_id"), actor_id=actor_id)
            await session.commit()
...
@require_auth("admin", "support", "auditor")
async def handle_web_knowledge_metrics_summary(_request: web.Request) -> web.Response:
    async with get_session() as session:
...
async def handle_web_knowledge_graph_nodes(request: web.Request) -> web.Response:
    ...
    async with get_session() as session:
        ...
        rows = (await session.execute(select(KnowledgeNode).order_by(KnowledgeNode.updated_at.desc()).limit(100))).scalars().all()
    ...
    async with get_session() as session:
        if request.method == "POST":
            if role not in {"admin", "support"}:
                return web.json_response({"status": "error", "error": "forbidden"}, status=403)
            payload = await _json_payload(request)
            result = await KnowledgeIngestionService(session).ingest_text(payload, actor_id=actor_id)
            await session.commit()
            return web.json_response({"status": "ok", **result})
        rows = (
            await session.execute(select(KnowledgeIngestionJob).order_by(KnowledgeIngestionJob.created_at.desc()).limit(100))
        ).scalars().all()
        return web.json_response(
            {
                "status": "ok",
