...
- `/app/admin/observer` trace detail показывает `GET /api/admin/tech/diagnostics/bundle` с next checks, logs/audit counters и компактными agent action rows; server-side action compaction обязан ограничивать большие `details` payloads, а materialized action-span sync включается только через `sync_agent_actions=1`, чтобы `tool response` traces не подвешивали detail/bundle UI.
