...
Integrity scans are fail-closed by source. Checkers return `ObserverIntegrityCheckResult` with an explicit `complete` status; bounded queries use a `LIMIT + 1` sentinel and process only the configured window, so an overflowed sample cannot resolve existing `active`/`acknowledged` events outside the page. Missing runtime state, checker failures, timeouts and legacy/plain-list checker results are treated as incomplete for resolution. Each checker is bounded by the runner timeout, gets a per-check report with status, started/finished/duration, generated/active/suppressed/resolved counts, scanned count, limit/window and redacted error details, and the report is persisted in `observer_integrity_check_runs` under the scan `scan_id`. A failed or timed-out checker creates an `observer.integrity_runner` self-health event, and scan API responses include `scan_id`, overall status, `checks`, `incomplete_sources` and `failed_sources`. Repeated observations preserve operator `acknowledged` state; only a checker report with `status=passed` and `complete=true` may resolve that source.
