...
- MCP presence snapshots report persisted `device_presence_snapshots` when available and include top-level DB `Device.last_seen_at` / `last_handshake_at` evidence for an explicit `device_id` even when no persisted presence snapshot exists; this remains historical DB evidence, not live websocket state.
