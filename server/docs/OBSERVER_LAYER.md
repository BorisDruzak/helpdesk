...
OBS1 Operational Integrity Observer adds a durable integrity-event stream on top of the trace overlay. It does not replace traces and does not mutate product state. It persists runtime invariant violations with `dedupe_key`, severity, source, status, `scan_observation_count`, `recurrence_count`, backward-compatible `occurrence_count` as a recurrence alias, `first_seen_at`, `last_seen_at`, `last_reopened_at`, runbook, redacted evidence, optional correlation ids (`device_id`, `ticket_id`, `operation_id`, `command_id`, `device_outbox_id`, `trace_id`) and resolution/suppression state. Known P0-P6 contamination is represented narrowly in `observer_known_contamination` by exact entity ids or stable dedupe keys. Active known-contamination seed data is governed by `quality/observer_known_contamination.json` and `scripts/audit_observer_contamination.py`; rows without a future `expires_at` do not suppress integrity events.

Observer volume query plans have a schema-level contract in `server/tests/test_observer_query_plan_no_db.py`. Retention cleanup must stay backed by `observer_traces(status, started_at)`. Trace search/order paths must keep composite trace indexes for `root_kind`, `ticket_id`, `device_id`, `operation_id` and `job_id` with `started_at`; trace detail and span filters must keep `observer_spans(trace_id, started_at|tool_name|module_name|event_type)`; error detail and ticket signature stats must keep `observer_error_occurrences(trace_id, created_at|error_kind|error_signature)` and `observer_error_occurrences(ticket_id, error_signature, created_at)`. New Observer search filters need the same model + Alembic coverage before relying on them at volume.

Integrity scans are fail-closed by source. Checkers return `ObserverIntegrityCheckResult` with an explicit `complete` status; bounded queries use a `LIMIT + 1` sentinel and process only the configured window, so an overflowed sample cannot resolve existing `active`/`acknowledged` events outside the page. Missing runtime state, checker failures, timeouts and legacy/plain-list checker results are treated as incomplete for resolution. Each checker is bounded by the runner timeout, gets a per-check report with status, started/finished/duration, generated/active/suppressed/resolved counts, scanned count, limit/window and redacted error details, and the report is persisted in `observer_integrity_check_runs` under the scan `scan_id`. A failed or timed-out checker creates an `observer.integrity_runner` self-health event, and scan API responses include `scan_id`, overall status, `checks`, `incomplete_sources` and `failed_sources`. Repeated observations preserve operator `acknowledged` state and increment only `scan_observation_count`; `recurrence_count` and the legacy `occurrence_count` alias increment only when a previously resolved condition returns and sets `last_reopened_at`. Only a checker report with `status=passed` and `complete=true` may resolve that source.
...
`GET /api/web/admin/observer/integrity` is admin/support/auditor readable and supports filters by `severity`, `status`, `device_id`, `ticket_id`, `operation_id`, `event_type`, `source`, `since` and `limit`. Integrity event payloads expose `scan_observation_count`, `recurrence_count`, `last_reopened_at` and legacy `occurrence_count` as a recurrence alias. `POST /api/web/admin/observer/integrity/scan` is admin/support and runs the OBS1 checkers, returning `scan_id`, `status`, `duration_ms`, event counts, `incomplete_sources`, `failed_sources`, `event_ids` and per-check `checks` reports. Requester/public/agent tokens must be denied.
...
Web-cabinet trace identity prefers durable retry keys (`idempotency_key`, `operation_id`) when a logical action exposes them, then the server-generated `server_request_id` that every HTTP request receives, and only then client-provided `request_id` / `correlation_id`, before entity ids such as `ticket_id`. Two independent actions on the same ticket must not collapse into one trace even when the browser omits `X-Request-ID`; a retry with the same durable execution key may update the same trace while preserving error occurrences/spans. Requester ticket-create integrity coverage is successful only when a trace has `root_kind=requester_web`, `source=requester_ticket_create`, a success create event type, `status=succeeded`, and a success result.

Typed admin observer trace detail also exposes an explanation projection through `explanation` on `GET /api/web/admin/observer/traces/{trace_id}` and the compatibility alias `GET /api/web/admin/observer/trace-detail/{trace_id}`. This projection is the operator-facing layer above raw spans: `launch_source` / `launch_source_label`, actor fields, tool/module labels, preset label/description, `error_code`, human diagnosis, launch path, next actions, agent online label and debug refs. The raw `spans`, `span_links`, `error_occurrences`, agent actions and attrs remain present for debug mode. If a hot/projected trace id no longer has persisted detail, the web boundary returns a typed empty detail payload (`status=missing`, zero spans/errors) instead of an HTTP 404, so `/app/admin/observer` does not turn expected projection gaps into browser-visible network errors.

Operation stage spans must describe the stage outcome, not blindly mirror the terminal operation status. Intermediate stages such as `queued`, `sent`, `accepted` and `running` are `ok`/`running` when they completed before a later terminal failure; the final failed/timed-out/denied stage carries `status=error`, `stage_state=failed` and `is_failure_stage=true`. UI should render the explanation block before technical spans, then show stage labels such as `Queued` or `Failed` with raw names (`operation.stage.queued`) as secondary debug detail.
...
- Standalone MCP server `helpdesk-server-debug` exposes the same observer read model to Codex through stdio tools (`observer_debug_bundle`, `observer_trace_detail`, `observer_ticket_summary`) using `server/observer/debug_facade.py`. It is `debug_readonly`: no HTTP API proxying, no `run_tool`, no DeviceOutbox writes, no approvals/consent decisions, no WS RPC and no raw-secret output. The debug facade applies an additional MCP output redaction layer for public ticket access-code fields and text snippets before returning observer payloads.
- The live aiohttp server writes fresh `server_runtime_snapshots` rows through `server/observer/runtime_snapshot_writer.py`; MCP `observer_runtime_status` reads those persisted rows instead of importing live process memory.
- MCP presence snapshots report persisted `device_presence_snapshots` when available and include top-level DB `Device.last_seen_at` / `last_handshake_at` evidence for an explicit `device_id`; when a fresh server runtime snapshot contains connected agents, `live_ws_evidence` exposes bounded aggregate websocket state without `device_id` and device-specific websocket state with `device_id`, never raw websocket objects or tokens.
...
Typed support detail (`GET /api/web/support/tickets/{ticket_id}`) embeds a compact observer payload for `/app/tickets`: root trace status/url, root kind, health label (`empty`, `ok`, `running`, `error`), latest error label/stage/time, top ticket-local signature, compact related/active/error trace rows, compact recent occurrences, `web_flow` status for profile gate / preview / create / target / knowledge guard / requester chat / requester closure / support chat / support status / support assignment, and active `observer.web_cabinet` integrity events with safe trace URLs plus observation/recurrence counters. Requester ticket-create diagnostic target events (`diagnostic_target_missing|diagnostic_target_offline|diagnostic_target_ambiguous`) feed the compact target web-flow status while full trace/span payloads stay in `/app/admin/observer`. Full spans, span links, raw occurrence detail and raw integrity evidence stay in `/app/admin/observer`.
...
- `/app/admin/observer` trace detail must lead with `explanation`: launch source, actor, tool/module/preset label, human error diagnosis, launch path and next actions. Technical spans, UUIDs and raw attrs stay available below this summary. Missing projected detail is a typed empty state, not a failed network request.
- `/app/admin/observer` trace detail показывает `GET /api/admin/tech/diagnostics/bundle` с next checks, logs/audit counters и компактными agent action rows; long `error_signature`, trace and operation identifiers must wrap inside drilldown cards; server-side action compaction обязан ограничивать большие `details` payloads, а materialized action-span sync включается только через `sync_agent_actions=1`, чтобы `tool response` traces не подвешивали detail/bundle UI. Bundle endpoint returns an empty `status=ok` bundle for missing projected traces so React Query does not emit console/network errors during live Observer signoff.
