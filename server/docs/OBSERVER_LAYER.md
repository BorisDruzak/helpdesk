...
- MCP presence snapshots report persisted `device_presence_snapshots` when available and include top-level DB `Device.last_seen_at` / `last_handshake_at` evidence for an explicit `device_id`; when a fresh server runtime snapshot contains the device, `live_ws_evidence` exposes bounded live websocket state without raw websocket objects or tokens.
