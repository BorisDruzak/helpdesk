...
- `/app/admin/knowledge/sections`: Knowledge Section Constructor. It lists `KnowledgeSpace` rows as `Разделы базы знаний`, edits title/code/description/status/default visibility plus publication/import/RAG flags, and embeds a section-level Registry audience editor. Audience preview/save calls must use `subject_type=space` and the selected `space_id`; this screen must stay Russian-first and hide raw JSON/UUIDs from the normal workflow.
