...
- `/app/admin/knowledge/import`: dedicated Import/Ingestion wizard for text, markdown and HTML preview plus AI-off review draft creation through the import preview/create-drafts APIs. The backend preview contract already accepts safe DOCX/PDF base64 uploads, fail-closed URL/Git source kinds, optional non-AI auto segmentation after draft creation and indexing metadata that runs the existing item indexing path when `vector_enabled=true`; UI upload/profile/indexing controls and AI enrichment proposal review remain later Phase 10 slices.
