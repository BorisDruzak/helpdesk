...
- `/app/admin/knowledge`: Russian-first `Центр операций базы знаний` landing route. The dashboard reads `GET /api/web/knowledge/ops/summary` for coverage, quality, search/RAG, indexing, AI, graph, review and Observer-backed degradation cards, and `GET /api/web/knowledge/metadata` for taxonomy/property/applicability/quality-model counts plus active quality weights. It provides quick transitions to dedicated Sections, Studio, Import, Graph, Search Settings, Indexing and AI routes instead of mixing governance/editor controls into the ops landing page.
