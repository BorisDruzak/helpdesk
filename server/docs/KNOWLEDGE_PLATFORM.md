...
Published items must have a current version. The backend publish contract remains explicit: callers publish with a concrete `version_id`, and `knowledge_items.current_version_id` is only the current published pointer and is not required before the first publish. The simplified Studio default UI now wraps this into one `Сохранить статью` action: create a new `knowledge_item_versions` row from the form, publish that version as current, refresh the article list, and keep rollback inside `История версий`. When `KNOWLEDGE_REVIEW_REQUIRED=false` (the default simplified-mode policy), publication does not fail solely because `reviewer_actor_id` is empty; `KnowledgeRepo.publish_item` autofills it from the current actor or `servicedesk`. Owner, requester-safety, stale passport and explicit `metadata.review_required` blockers still apply.
...
Registry-scoped audience rules are tracked in [REGISTRY_VISIBILITY_FOUNDATION.md](REGISTRY_VISIBILITY_FOUNDATION.md). The first Phase 5 backend slices add `knowledge_audience_rules`, shared `KnowledgeAccessService`, `KnowledgeAudienceRulesService`, admin-only audience-rule list/replace/preview/explain routes under `/api/web/admin/knowledge/*`, HTTP search/suggestion enforcement through `KnowledgeSearchService.search(..., effective_audience=...)`, direct requester article-detail denial before body/segment projection, requester portal home/space/tag list filtering before summary projection, requester portal action denial before feedback/correction/bookmark writes, Ask/RAG/vector retrieval candidate filtering through `KnowledgeRetrievalService.retrieve(..., effective_audience=...)` before ordering/rerank/citations/answer prompt construction, and support ticket knowledge-suggestions filtering for requester-safe P2 articles through the ticket requester audience while preserving `support_internal` runbooks from the support context. Phase 5 live anti-leak evidence is recorded by `scripts/knowledge_audience_live_smoke.py` in `artifacts/browser_live_validation/phase5-knowledge-audience-20260614145845/live-smoke-report.json`: requester effective-audience behavior is proven at service level, while support ticket suggestions and admin explain denial are proven over live HTTPS. Audience rules refine this coarse visibility model after role/status checks. They must not downgrade support/admin-only content into requester-visible content, and requester/agent/public APIs must not leak denied article titles, summaries, chunks, counts, diagnostics or rule metadata.
...
K1 section metadata is intentionally narrow and each key has a single owner plus a runtime reader:

- `show_in_requester_portal`: owned by `/app/admin/knowledge/sections` and validated by `KnowledgeRepo.upsert_space`. `KnowledgePortalService` reads it for portal home, space collections and direct article reads; `false` hides the section and its articles from requester portal surfaces without granting or weakening visibility. `true` is accepted only for requester-safe section visibility.
- `show_in_support_workspace`: owned by `/app/admin/knowledge/sections`. `/app/knowledge` reads it through `KnowledgeSupportWorkspacePage` after fetching spaces; `false` hides section materials from the support knowledge workspace list without changing underlying item ACLs or management APIs.
- `article_length_recommendation`: owned by `/app/admin/knowledge/sections`. `EditorMetadataStep` reads it in Studio as an authoring hint for the selected section. Supported values are `short`, `standard` and `detailed`; unknown legacy strings are displayed as-is rather than treated as policy.

Section policy validation is fail-closed for contradictions. `KnowledgeRepo.upsert_space()` rejects explicit empty `allowed_item_types`, rejects `allow_rag=false` when an active article in the section forces `metadata.ai_rag_policy=allowed`, and keeps legacy/new sections with omitted item types on the default `article`, `faq`, `runbook` set. `KnowledgeRepo.create_item_draft()` and `update_item_settings()` reject article `ai_rag_policy=allowed` when the target section has `allow_rag=false`.
...
2026-06-15 R9 update: pre-submit requester suggestions can also use safe values from R8 `form_payload`, `requester_context` and selected `device_metadata` as search signals. `KnowledgeSuggestionService` skips raw ids, token/secret/session/cookie/password fields, email, phone and other sensitive identifiers while still passing every candidate search through `KnowledgeSearchService.search(..., effective_audience=...)`. The same Registry audience decision remains shared by requester search, suggestions, portal actions, Ask/RAG/vector retrieval and support ticket requester-attempt context before any title, snippet, citation or diagnostic projection.
...
Studio also shows a client-side eligibility preview in the binding panel. It uses the current draft visibility, selected binding surfaces, article RAG policy, selected section `allow_rag` and the existing audience-rule estimate to split surfaces into `Статья будет предложена в` and `Не будет предложена в` with explicit reasons: `visibility`, `audience`, `surface disabled` or `RAG disabled`. This is an authoring explanation only; backend search/suggestion/retrieval gates remain the enforcement source.
...
2026-06-16 PA7: on-behalf requester context may add `affected_context` as a sanitized suggestion query source, but it does not change the requester effective audience. Creator-facing suggestions still pass through `KnowledgeSearchService.search(..., effective_audience=<creator audience>)` before any projection, so affected-person department or service facts cannot grant restricted articles to the creator. `knowledge_attempts` record `visibility_scope` and `audience_scope`; requester and agent-created attempts are server-scoped to `creator_visible` / `creator`, while `support_only` / `affected_context` attempt metadata is reserved for support workspace flows.
...
- Phase 12 adds `KnowledgeOpsSummaryService` and `GET /api/web/knowledge/ops/summary` as the first Knowledge Operations Center snapshot. It aggregates existing coverage, quality, search/RAG, indexing, AI, graph, review and Observer v2 degradation signals without adding new tables. The endpoint is the dashboard contract for `/app/admin/knowledge` and includes safe `action_queues` for no-audience articles, missing helpdesk binding, stale articles, indexing failures, low quality and zero-result searches; deeper event emission and richer Observer mappings remain later Phase 12 slices.
- Phase 14 adds `KnowledgeMetadataService` and `GET /api/web/knowledge/metadata` for governed taxonomy, property definitions, item metadata, applicability rules and quality models. The bundle returns all visible management rows plus `summary` total/active counts; Knowledge Ops uses active counts so draft/archived taxonomy terms and properties do not count as active coverage. Metadata write routes require the explicit `knowledge.metadata.manage` permission in addition to the authenticated role boundary: `POST /api/web/knowledge/taxonomy`, `POST /api/web/knowledge/properties`, `PUT /api/web/knowledge/items/{item_id_or_slug}/metadata`, `POST /api/web/knowledge/items/{item_id_or_slug}/applicability` and `POST /api/web/knowledge/quality-models`. Admin receives this permission by default; support must be explicitly delegated as a knowledge manager through access groups and does not receive metadata-management writes by default. Taxonomy term upsert validates both the parent space visibility and the requested term visibility. Item metadata reads filter assigned taxonomy terms by the actor's visible visibilities, and updates reject taxonomy term assignment when the actor cannot mutate that term visibility. Auditor can read the bundle; requester/public APIs do not receive this admin metadata bundle or quality model diagnostics.
...
- `/app/admin/knowledge`: Russian-first `Центр операций базы знаний` landing route. The dashboard reads `GET /api/web/knowledge/ops/summary` for coverage, quality, search/RAG, indexing, AI, graph, review, Observer-backed degradation cards and actionable queues, and `GET /api/web/knowledge/metadata` for taxonomy/property/applicability/quality-model counts plus active quality weights. It renders queue labels such as `Статьи без аудитории`, `Статьи без связи с обращениями`, `Просроченные статьи`, `Ошибки индексации`, `Низкое качество` and `Поиски без результата`, with next actions to Studio article, Sections, Indexing, Search Settings or Import/create article. Quick transitions include Russian hover hints. The route stays operations-only and does not expose content-pack/raw JSON/debug workflows by default.
- `/app/admin/knowledge/sections`: Knowledge Section Constructor. It lists `KnowledgeSpace` rows as `Разделы базы знаний`, edits title/code/description/status/default visibility plus publication/import/RAG flags, shows article counts and per-list audience summaries, and embeds a section-level Registry audience editor. Audience preview/save calls must use `subject_type=space` and the selected `space_id`; list summaries may use `GET /api/web/admin/knowledge/audience-rules?subject_type=space` and must stay Russian-first without raw JSON/UUIDs in the normal workflow.
- `/app/admin/knowledge/metadata`: dedicated metadata management workbench for governed taxonomy terms, property definitions, applicability rules and quality models. The route requires admin workspace access plus `knowledge.metadata.manage`; support users can open it only when explicitly delegated as knowledge managers through access groups. Editors create, edit and archive metadata through typed Russian-first forms; auditor remains read-only through the backend, and requester navigation must not expose this route.
- `/app/admin/knowledge/studio`: simplified Knowledge Authoring Studio route with draft/article browser, new draft creation, article main fields, Russian explanations for `Раздел базы знаний`, `Тип материала`, `Кому доступна статья`, `Аудитория` and `Где показывать статью`, Registry-backed audience-rule selector, helpdesk context panel `Связь с обращениями`, simple read-only graph panel `Связанные статьи`, TipTap article editor, template insertion, structured Markdown block insertion, requester-safe preview, one primary `Сохранить статью` action, save-readiness checks and persisted editor history/diff cache. The binding panel links the selected article to service/offering/request-template/ticket-type context through `GET|POST /api/web/knowledge/items/{item_id_or_slug}/bindings`, stores display surfaces in binding metadata and hides raw binding ids. The related-articles panel uses existing graph search/neighborhood reads when the selected article has a graph node; loading, empty and failed graph states are non-blocking and do not affect the normal save flow. Default UI hides review actions, manual version plumbing, manual segmentation and advanced metadata; manual/auto segmentation remains under advanced tools, and rollback/version selection is available only in the `История версий` drawer with confirmation.
- `/app/admin/knowledge/graph`: dedicated Visual Graph Studio route with Russian-first node search, SVG neighborhood canvas, node inspector, selected-node label edit, manual node creation, edge creation, edge/node archive controls, pending graph AI proposal review and persisted canvas layout save/load over graph node/edge/layout/proposal APIs. K8 keeps this route as the advanced workbench for related articles, duplicates, supersedes, known error/workaround and service/article relations; ordinary article creation and saving do not require graph work.
...
K7 product cleanup makes `/app/admin/knowledge/search-settings` explain keyword/full-text/vector/RAG differences, RAG eligibility, and visibility/audience filtering before ranking, snippets and citations. `/app/admin/knowledge/ai` shows provider health, RAG policy blocks, requester-safe cloud policy, the security-restricted local requirement and redacted audit posture. These settings surfaces are explanatory and operational; they do not grant visibility or bypass section `allow_rag`, article `metadata.ai_rag_policy`, audience rules or requester-safe projection.

K8 graph simplification keeps Knowledge Graph as an advanced workbench and adds a lightweight Studio `Связанные статьи` panel. Studio reads existing graph search/neighborhood endpoints only to show linked article nodes near the selected article; graph loading, empty results and graph API failures remain non-blocking and do not affect the one-save authoring flow. Full node/edge creation, duplicates, supersedes, known error/workaround and service/article relation work stays in `/app/admin/knowledge/graph`.
...
2026-06-16 PA7 Ask/RAG boundary: requester `POST /api/knowledge/ask` continues to force requester role and must receive the creator effective audience from the HTTP/auth boundary. `KnowledgeRetrievalService.retrieve(..., effective_audience=...)` filters candidates before fallback result projection and before the RAG prompt is built; affected-person context from on-behalf tickets is not an audience grant and must not place restricted snippets, titles or citations into the requester prompt.
...
Registry Visibility Foundation Phase 6 adds the first authoring selector for `knowledge_audience_rules` in React `/app/admin/knowledge/studio`. The settings step renders `Аудитория`, shows the coarse article visibility, loads persisted item audience rules, offers Registry-backed allow-rule pickers for role, person, department, department tree, location, access group, audience group and service, estimates matched Registry people, warns on broad, empty or internal-only combinations, calls `POST /api/web/admin/knowledge/audience-rules/preview` and `GET /api/web/admin/knowledge/access/explain`, and saves through `PUT /api/web/admin/knowledge/audience-rules` with a reason. This UI is an admin authoring/debug surface only; requester, agent, public and support read APIs still rely on server-side `KnowledgeAccessService` enforcement and must not trust frontend filtering.
