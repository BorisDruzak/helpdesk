...
- Knowledge Platform management endpoints under `/api/web/knowledge/*` are web-session protected. Admin can manage spaces/items/versions/graph/ingestion; support may use support-ticket knowledge surfaces and drafts according to existing support workspace RBAC; auditor is read-only where exposed. Requester/agent safe endpoints `POST /api/knowledge/search`, `POST /api/knowledge/suggest` and `POST /api/knowledge/feedback` are intentionally auth-whitelisted for the public `/app/help` deflection flow, default anonymous calls to requester-safe scope, and apply visibility filtering before results. They must not return internal bodies, support/admin/security-restricted items, source ticket/passport ids, requester/device ids, raw custom fields, internal graph edges, queue/policy ids, trace ids, operation ids or restricted chunks. Passport-to-knowledge creates drafts only and never publishes without review.
