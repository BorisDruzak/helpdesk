...
- Service Catalog admin mutation endpoints under `/api/web/admin/service-catalog*` require `admin`; auditor may read, validate and simulate, but cannot mutate. The requester/agent projection under `/api/service-catalog/*` is safe for catalog selection and does not include queue ids, raw policy JSON, approver internals, requester ids, device ids, raw custom fields, trace ids or operation ids.
