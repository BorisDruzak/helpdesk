...
5. **Критично:** `device_id` берётся **только из записи токена в БД**, не из payload. Если в payload указан другой `device_id` — используется device_id из токена и пишется предупреждение в лог.
6. После успешной проверки создаётся `AuthContext(actor_id=device_id, actor_role="agent", auth_type=AuthType.AGENT_TOKEN)` и сохраняется в метаданных соединения.
