...
- Cutover legacy shell управляется флагами `WEBAPP_CUTOVER_LOGIN_ENABLED`, `WEBAPP_CUTOVER_SUPPORT_ENABLED`, `WEBAPP_CUTOVER_ADMIN_ENABLED` в `server/config.py`; при включённом cutover старые `/login`, `/support`, `/admin` редиректят в `/app/*`, но явный escape на legacy shell сохраняется через `?legacy=1`.
...
- Каноничный preflight перед полным переключением: `python scripts/check_webapp_cutover.py --json`; каноничный live signoff после release: `pnpm --dir webapp run check:remote -- --base-url http://192.168.100.17:8666`, который теперь подтверждает и raw redirects `/login|/admin|/support`, и `?legacy=1` escape, а в день полного cutover может быть ужесточён флагом `--expect-route-mode webapp`.
