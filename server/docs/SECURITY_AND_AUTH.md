...
- Каноничный preflight перед полным переключением: `python scripts/check_webapp_cutover.py --json`; каноничный live signoff после release: `pnpm --dir webapp run check:remote -- --base-url http://192.168.100.17:8666`, который теперь подтверждает и raw redirects `/login|/admin|/support`, и `?legacy=1` escape.
