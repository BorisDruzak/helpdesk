...
- Ticket create builds a server-owned `ticket_context_v1` snapshot in `server/tickets/ticket_context.py`. The canonical snapshot includes `created_at`, creator, affected person, requester context, diagnostic target, form snapshot, policy references and redaction metadata. Agent-originated requester scope is derived from the active device binding; browser-without-device requests are resolved from the authenticated web identity. Legacy `ticket.device_id` and `requester_*` database columns remain read-only compatibility data. Requester/public projections hide raw identifiers, policy references and diagnostic source codes; support detail exposes typed `ticket_context`.
