...
- Policy Health checks Service Catalog knowledge gaps: a published public service/offering with no requester-safe published knowledge binding gets a warning. P2.2 adds operations endpoints for review queue, quality score, content packs, Service Catalog gap metrics and controlled requester/agent deflection rollout; ticket create, `knowledge_attempts` and `kb_links` compatibility stay unchanged. Full model, API, ACL and rollback contract: [KNOWLEDGE_PLATFORM.md](KNOWLEDGE_PLATFORM.md).
