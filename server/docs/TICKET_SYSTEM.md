...
- Ticket create builds a server-owned `ticket_context_v1` snapshot in `server/tickets/ticket_context.py`. The canonical snapshot includes `created_at`, creator, affected person, on-behalf state, requester context, diagnostic target, form snapshot, policy refs and redaction metadata while preserving legacy aliases under `target_device` and flat custom fields (`creator_person_id`, `affected_person_id`, `target_device_id`, `target_agent_status`, etc.). Normal tickets use creator=affected and source `creator_primary_agent`; on-behalf tickets use the affected user's primary agent and source `affected_user_primary_agent`; missing/offline/ambiguous targets are evidence (`no_primary_agent`, `ambiguous_primary_agent`, target-agent status), not a fallback to the creator's current PC. Ticket create writes `ticket_context_resolved` as internal/support evidence. Legacy `ticket.device_id` and `requester_*` fields remain compatibility fields. Requester/public projections hide raw ids, policy refs and diagnostic source codes; support detail exposes typed `ticket_context`.
