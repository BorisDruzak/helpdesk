...
- Ticket create builds a server-owned `ticket_context_v1` snapshot in `server/tickets/ticket_context.py`. It stores creator, affected person, `created_on_behalf`, resolved diagnostic target and `diagnostic_target_source` in `custom_fields.ticket_context` plus flat aliases (`creator_person_id`, `affected_person_id`, `target_device_id`, `target_agent_status`, etc.). Normal tickets use creator=affected and source `creator_primary_agent`; on-behalf tickets use the affected user's primary agent and source `affected_user_primary_agent`; missing/ambiguous targets are evidence (`no_primary_agent`, `ambiguous_primary_agent`), not a fallback to the creator's current PC. Legacy `ticket.device_id` and `requester_*` fields remain compatibility fields. Requester/public projections hide the raw ticket context; support detail exposes it through typed `ticket_context`.
