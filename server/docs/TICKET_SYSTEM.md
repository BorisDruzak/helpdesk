...
- Request-template forms may also carry `on_behalf_policy`. It is disabled when absent, normalized by `server/tickets/form_catalog.py`, exposed through the admin forms DTO and projected to web settings as a safe capability object. The policy only enables/configures the future affected-person requester flow; ticket creation still relies on the server-owned `ticket_context_v1` resolver for creator, affected person and diagnostic target snapshots.
