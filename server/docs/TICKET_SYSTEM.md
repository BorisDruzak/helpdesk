...
- Admin: GET/POST/PATCH /api/admin/tickets/calendars; GET/PUT /api/admin/tickets/queues/{id}/ola_targets. В POST/PATCH sla_policies — calendar_id. GET /api/web/settings accepts historical dict-shaped and list-shaped `weekly_hours_json` / `holidays_json` calendar rows. GET /api/tickets/{id}/sla — при TICKET_OLA_ENABLED блок **ola** в ответе. GET /api/web/support/queue returns `summary.smart_view_counts` so server UI can render OLA-risk operational slices without client-side recomputation.
