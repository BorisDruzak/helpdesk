...
- Authenticated public ticket claim uses `POST /api/web/requester/tickets/claim-public`. It verifies the public access code, requires the web user to resolve to a `RegistryPerson`, and attaches the ticket with an atomic conditional update on `tickets.requester_person_id` so concurrent different-person claims are single-winner. The winning transaction clears the public unbound marker and writes one `requester_ticket_claimed` event without storing the access code; the losing transaction returns `PUBLIC_TICKET_ALREADY_CLAIMED`/409. Unlinked web users receive `REQUESTER_IDENTITY_REQUIRED`; the system does not create web-login-only claims.
- Public ticket code-to-session exchange uses `POST /public_api/tickets/{ticket_id}/authorize`. The handler applies the shared auth rate limiter before checking the public access code: 5 attempts per 300 seconds per `client_ip:ticket_id`, returning HTTP 429 / `RATE_LIMITED` after the limit. This is the abuse cooldown for the 8-character public access code before a scoped public session token is minted.
...
## PR-6 External Knowledge boundary (2026-08-09)

- Helpdesk has no in-process Knowledge Platform. The former `/api/knowledge/*`, `/api/web/knowledge/*`, Knowledge UI routes and local AI runtime are removed and return normal `404` where applicable.
- Ticket and support code use `KnowledgePort` only. The default adapter is fail-closed: the support workspace exposes `knowledge.status=unavailable`, `code=knowledge_unavailable` and no suggestions; ticket creation, routing, support work and closure continue normally.
- `ticket_kb_links` rows and sanitized legacy `custom_fields.knowledge_attempts` are read-only history. Helpdesk has no KB-link mutation endpoint and does not create new local Knowledge state.
- PR-7 may connect the versioned external API defined in [KNOWLEDGE_PLATFORM_API_V1.md](KNOWLEDGE_PLATFORM_API_V1.md) without a local fallback. PR-11 may delete the retained physical schema only through an accepted forward-only migration after external-service acceptance.
...
- Ticket create builds a server-owned `ticket_context_v1` snapshot in `server/tickets/ticket_context.py`. The canonical snapshot includes `created_at`, creator, affected person, on-behalf state, requester context, diagnostic target, form snapshot, policy refs and redaction metadata while preserving legacy aliases under `target_device` and flat custom fields (`creator_person_id`, `affected_person_id`, `target_device_id`, `target_agent_status`, etc.). Normal tickets use creator=affected and source `creator_primary_agent`; on-behalf tickets use the affected user's primary agent and source `affected_user_primary_agent`; missing/offline/ambiguous targets are evidence (`no_primary_agent`, `ambiguous_primary_agent`, target-agent status), not a fallback to the creator's current PC. Ticket create writes `ticket_context_resolved` as internal/support evidence, `customer_history_ticket_created` as a safe Customer History marker and `requester_ticket_create_audit` as an internal audit event. The B3 marker/audit payloads use `source=requester_ticket_create` and safe request-context/account-mode/snapshot/diagnostic-target flags only, not raw requester login, title, description, person ids or device ids. Account-session create paths keep verified `confirmed_binding` / `verified_other_account` requester context; legacy create paths without `requester_account` use `requester_account_mode=agent_legacy_or_device_only`, `context_scope=limited` and `profile_completion_evidence=false` so support can distinguish compatibility tickets from web-profile-complete tickets. Legacy `ticket.device_id` and `requester_*` fields remain compatibility fields. Requester/public projections hide raw ids, policy refs and diagnostic source codes; support detail exposes typed `ticket_context`.
- Authenticated requester on-behalf creation is policy-gated. `GET /api/web/requester/on-behalf/people` searches only within the selected form's `on_behalf_policy.allowed_scope`; `POST /api/web/requester/tickets/preview` and `POST /api/web/requester/tickets` revalidate `ticket_context.affected_person_id` and reason server-side before any preview/create result. The requester UI does not choose a diagnostic device for on-behalf tickets; `ticket_context_v1` resolves the affected person's primary agent or records missing/ambiguous evidence.
- Diagnostic/module execution resolves the runtime target through `server/tickets/diagnostic_target.py`. Auto-run playbooks, diagnostic policies, support manual tools/playbooks and `/api/tools/run` prefer `custom_fields.ticket_context.diagnostic_target.device_id`, then the flat `custom_fields.target_device_id`, then legacy `ticket.device_id` only for tickets without a context snapshot. Offline/missing/ambiguous targets produce `diagnostic_autorun_skipped` evidence instead of dispatching to the creator's current device, and support tools/playbooks payloads include safe `diagnostic_target` metadata for the operator UI.
- Customer History v1 lives in `server/customer_history/*` as a product read model over tickets, ticket events, chat messages, Registry bindings/sessions, diagnostics operations, compact `ObserverTrace` summaries, SLA fields and sanitized legacy Knowledge-attempt metadata. Support/admin routes are `GET /api/web/support/people/{person_id}/history`, `GET /api/web/support/tickets/{ticket_id}/history`, `GET /api/web/support/tickets/{ticket_id}/context-pack`, `POST /api/web/support/tickets/{ticket_id}/llm-context/preview` and `GET /api/web/admin/history/search`; requester-safe routes are `GET /api/web/requester/history` and `GET /api/web/requester/tickets/{ticket_id}/history`. Projections are role-aware and recursively redact passwords, tokens, cookies, sessions, auth headers, raw metadata, access/pairing codes, trace/span attrs and large attachment bodies; requester/LLM projections include only the redacted historical shape, never local Knowledge content. On-behalf tickets appear in creator and affected person history with relationship markers, and history uses safe ticket/operation/observer refs instead of raw trace/operation ids for linkable context. Support ticket detail embeds compact `customer_history` and `llm_context_preview`; the preview is deterministic, bounded, can append related recent history after the current ticket and never calls an LLM API.
...
- Versioned `form_schemas`, `form_fields` and `form_conditions` live in migration `067`. `request_templates.form_schema_id` is now backed by a first-class schema publication when the visual forms builder publishes a template; the legacy `ticket_form_packs` path stays compatible for `/help`, agent create and old packs. `server/tickets/request_template_submission.py` lets authenticated create, create-preview and public create validate either a legacy `request_forms` entry or an active standalone registry `request_template` + `form_schema` when `request_template_key` points at the helpdesk model registry. The resolver now returns source metadata (`resolved_from`, pack/template/schema keys and versions) so create/create-preview can explain whether runtime used `legacy_pack` or `standalone_registry`. Field `process_mapping.roles` is normalized as an alias of existing `field_roles`, and `validation.required_message` is preserved by server-side submission validation. `server/tickets/form_catalog.py::validate_form_submission()` also enforces server-side dynamic field constraints for visible submitted fields: required values, option membership, number min/max, text min/max length, regex, email and http/https URL checks; hidden `visible_when` fields are omitted even if a forged payload sends them. When old preferred `request_forms` packs are normalized with default priority questions, those injected fields stay optional for old clients that submit only the original `form_key` payload.
- Public requester-safe `request_forms` projections may append a transient `+registry.N` suffix when standalone registry templates are merged into the preferred legacy pack for display. Preview/create handlers still resolve that value through `server/tickets/form_catalog.py::resolve_ticket_form_pack()` by falling back to the persisted base pack version before validating the submitted form payload; the transient suffix is not a stored `ticket_form_packs.version`.
...
- Request-template forms may also carry `on_behalf_policy`. It is disabled when absent, normalized by `server/tickets/form_catalog.py`, exposed through the admin forms DTO and projected to web settings as a safe capability object. The policy configures requester affected-person search/selection, required reason, allowed scope and no-primary-agent behavior; ticket creation still relies on the server-owned `ticket_context_v1` resolver for creator, affected person and diagnostic target snapshots.
...
### Watchers and retained KB history

- **POST/GET/DELETE** `/api/tickets/{id}/watchers` — requester только self в своём тикете. События: `watcher_added`, `watcher_removed`.
- `ticket_kb_links` remains physical historical data only. Its former `POST/GET/DELETE /api/tickets/{id}/kb_links` surface is removed; do not add local mutations before PR-11.
