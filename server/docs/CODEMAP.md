...
| `webapp/` | Новый React/Vite foundation для внутренних workspace-экранов `/app/support` и `/app/admin`: `src/main.tsx`, `src/app/router.tsx`, `src/app/providers/query-provider.tsx`, `src/app/layouts/app-shell.tsx`, `src/features/auth/*` с русским login/session shell; `src/shared/realtime/adapters/ws-ui-bridge.ts` и `src/shared/realtime/client.ts` прячут `GET /api/web/realtime/bootstrap`, `ui_hello`, ping/reconnect и live-only ticket/device subscriptions за единым typed client abstraction, а shared realtime client теперь откладывает idle-close на следующий tick, чтобы React subscription churn при переходах `/app/admin <-> /app/support` не закрывал `ws_ui` в состоянии `CONNECTING`; `src/features/queues/api.ts` + `src/features/queues/support-workspace.tsx` уже реализуют support workspace slice (queue/filter/selection, ticket timeline, быстрые статусы, composer ответа и компактный snapshot устройства/операций), получают live invalidation очереди/карточки через realtime bridge и теперь синхронизируют realtime ticket subscriptions диффом по стабильному множеству id, чтобы reorder/refetch очереди не пересоздавал подписки на всю видимую очередь и не засыпал `SubscriptionRegistry` сотнями повторных `subscribe_ticket`; а `src/features/admin/api.ts` + `src/features/admin/admin-workspace.tsx` вместе с `src/features/agent-updates/*`, `src/features/modules/*`, `src/features/forms-builder/*` и `src/features/tech/*` продолжают admin wave 1 через typed inventory устройств, rollout summary, русскоязычную карточку device update, typed registry модулей, forms-builder для каталога `request_forms`, device-scoped observer surfaces и live invalidation карточки устройства/observer quick/drilldown без legacy admin.js; `playwright.config.ts`, `tests/fixtures/support_fixture_server.py`, `tests/support-workspace.spec.ts` и `tests/admin-workspace.spec.ts` дают локальный browser smoke/signoff для русского `/app/support` и `/app/admin`, а `scripts/remote-browser-signoff.mjs` вместе с `pnpm --dir webapp run check:remote:webapp -- --base-url http://192.168.100.17:8666` стали canonical live signoff helper для Linux-стенда: проверяют `/app`, `/app/admin`, `/app/support`, session payload, русскую локаль, raw redirects `/login|/admin|/support`, `?legacy=1` escape и жёстко подтверждают webapp route mode поверх того же `server/static_pages/webapp_assets.py`, что используется сервером для выдачи built bundle |
