...
| `server/tickets/diagnostic_policy.py` | Executable request-template diagnostic policy helper: reads `custom_fields.request_template.diagnostic_policy` / legacy `diagnostics` and materializes terminal diagnostic operations into `ticket_evidence_items` when `attach_results.as_evidence=true` for passport/closure evidence |
