...
| `server/support.html`, `server/support.js`, `server/support.css` | Отдельный support workspace: двухрежимный экран `Очередь тикетов` + `Рабочий тикет`. В `Очереди тикетов` основным элементом является крупная доска тикетов с разворачиваемыми секциями `SLA / OLA / маршрут` и `Контекст и присутствие`, при этом в левом inbox фильтры `Нужны действия` и `Ждут пользователя` считаются только по своим тикетам, а неназначенные показываются отдельной нижней секцией. В `Рабочем тикете` используются режимы preview/work/observe и встроенный ticket workbench (`Контекст`, `Инструменты`, `Пайплайн`). Для закрытия тикета использует встроенную форму кодов решения вместо prompt. Dev-only тестовая учётка support для локальных/browser проверок: `op1` / `1.Abcdef` (убрать перед production) |
