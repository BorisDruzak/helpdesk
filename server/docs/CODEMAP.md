...
| `webapp/` | Новый React/Vite foundation для внутренних workspace-экранов `/app/support` и `/app/admin`: `src/main.tsx`, `src/app/router.tsx`, `src/app/providers/query-provider.tsx`, `src/app/layouts/app-shell.tsx`, `src/features/auth/*` с русским login/session shell; `src/features/queues/api.ts` + `src/features/queues/support-workspace.tsx` уже реализуют support workspace slice (queue/filter/selection, ticket timeline, быстрые статусы, composer ответа и компактный snapshot устройства/операций), а `src/features/admin/api.ts` + `src/features/admin/admin-workspace.tsx` вместе с `src/features/agent-updates/*`, `src/features/modules/*`, `src/features/forms-builder/*` и `src/features/tech/*` продолжают admin wave 1 через typed inventory устройств, rollout summary, русскоязычную карточку device update, typed registry модулей, forms-builder для каталога `request_forms` и device-scoped observer surfaces: быстрый срез трассировки, список последних trace и detail drilldown по spans/error occurrences поверх нового boundary без legacy admin.js; `playwright.config.ts`, `tests/fixtures/support_fixture_server.py`, `tests/support-workspace.spec.ts` и `tests/admin-workspace.spec.ts` дают локальный browser smoke/signoff для русского `/app/support` и `/app/admin` поверх того же `server/static_pages/webapp_assets.py`, что используется сервером для выдачи built bundle |
