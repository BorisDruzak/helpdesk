...
| `server/access_control/` | Server-owned RBAC catalog and resolver для нового Access Control Center: permission codes, Russian labels, risk groups, built-in role defaults, workspace derivation, `permissions_version` and effective-access calculation across role defaults + group grants. |
