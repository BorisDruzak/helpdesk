...
| `server/app/repos/helpdesk_policy_repo.py`, `server/tickets/helpdesk_policy_runtime.py` | Standalone versioned helpdesk model registry: publishes versioned `ticket_types`, `request_templates`, priority/SLA/OLA/routing/approval/closure/diagnostic/visibility/notification/reporting policies, smart views and audit rows; `ticket_types` carry default workflow/profile/policy references and feature flags that `request_templates` inherit during publication with legacy unknown-type fallback; resolves effective policy config through inheritance `system -> ticket_type -> category -> request_template`; supports policy and ticket-type version lookup, JSON config diff, soft deactivate and rollback-as-new-version with audit events; ticket creation overlays effective registry policies into `custom_fields.request_template`, while existing-ticket lifecycle helpers resolve effective approval/closure/notification/visibility/reporting policies without mutating historical snapshots; typed admin routes are `GET /api/web/admin/helpdesk-model/policies`, `POST /api/web/admin/helpdesk-model/ticket-types/publish|deactivate|rollback`, `POST /api/web/admin/helpdesk-model/request-templates/publish-from-form`, `POST /api/web/admin/helpdesk-model/policies/publish`, `POST /api/web/admin/helpdesk-model/policies/diff`, `POST /api/web/admin/helpdesk-model/policies/deactivate`, `POST /api/web/admin/helpdesk-model/policies/rollback` and `POST /api/web/admin/helpdesk-model/smart-views/publish`; active custom smart views are consumed by typed support queue runtime |
