...
| `server/tickets/statuses.py`, `server/tickets/workflow_service.py` | Product lifecycle: `new`, `queued`, `assigned`, `in_progress`, requester/internal/vendor/approval waits, `scheduled`, `resolved`, `closed`, `canceled`; `triaged` remains a legacy alias. Workflow updates `next_action_owner`, `requester_status`, `status_reason`, SLA pause/resume and `ticket_waits`; `ticket_to_dict` exposes requester-safe status labels and closure/evidence fields for support/requester UI. |
