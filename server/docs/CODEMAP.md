...
| `server/tickets/visibility_policy.py` | Executable request-template visibility policy helper: resolves `public_status` / `public_status_label`, exposes requester/support visibility metadata, and redacts requester-hidden fields from serialized ticket payloads |
| `server/tickets/notification_service.py` | In-app notification recipient resolver: legacy queue/assignee/requester/watchers delivery plus executable `request_template.notification_policy` event blocks (`on_status_changed`, `on_sla_breach`, etc.) before user notification preferences |
