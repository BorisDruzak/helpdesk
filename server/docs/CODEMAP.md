...
| `server/support.html`, `server/support.js`, `server/support.css` | Отдельный support workspace: двухрежимный `Queue Desk` + `Ticket Desk`, левый inbox тикетов, triage-сводка по SLA/OLA/непрочитанным, режимы preview/work/observe и встроенный ticket workbench (`Контекст`, `Инструменты`, `Пайплайн`). Для закрытия тикета использует встроенную форму кодов решения вместо prompt. Dev-only тестовая учётка support для локальных/browser проверок: `op1` / `1.Abcdef` (убрать перед production) |
