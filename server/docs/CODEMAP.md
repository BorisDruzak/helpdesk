...
| `server/tickets/statuses.py`, `server/tickets/workflow_service.py`, `server/tickets/workflow_profiles.py`, `server/tickets/priority_policy.py` | Product lifecycle: `new`, `queued`, `assigned`, `in_progress`, requester/internal/vendor/approval waits, `scheduled`, `resolved`, `closed`, `canceled`; `triaged` remains a legacy alias. `workflow_profiles.py` makes `ticket_type` a process selector for `incident`, `service_request`, `access_request`, `change_request` and `consultation`. Workflow updates `next_action_owner`, `requester_status`, `status_reason`, SLA pause/resume and `ticket_waits`; `priority_policy.py` calculates process priority P0..P3 from impact/urgency/importance facts and modifiers, while `sla_service.py` selects SLA targets by process priority with legacy P1..P4 fallback; `web_api/settings_handlers.py` exposes the full service-desk model surface (`process_schema`, planned support lines and priority model) for `/app/settings`; `ticket_to_dict` exposes requester-safe status labels, process priority and closure/evidence fields for support/requester UI. |
