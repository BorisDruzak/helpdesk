...
| `server/app/repos/helpdesk_policy_repo.py` | Standalone versioned helpdesk model registry: publishes `request_templates`, priority/routing/approval/closure/diagnostic/visibility/notification policies, smart views and audit rows; resolves effective policy config through inheritance `system -> ticket_type -> category -> request_template`; typed admin routes are `GET /api/web/admin/helpdesk-model/policies`, `POST /api/web/admin/helpdesk-model/request-templates/publish-from-form` and `POST /api/web/admin/helpdesk-model/policies/publish` |
