...
| `server/support.html`, `server/support.js`, `server/support.css` | Отдельный support workspace: inbox тикетов, режимы preview/work/observe, встроенный workbench drawer; drawer показывает состав очереди, факторы маршрутизации, SLA/OLA и состояние автоназначения очереди, а для закрытия тикета использует встроенную форму кодов решения вместо prompt. Dev-only тестовая учётка support для локальных/browser проверок: `op1` / `1.Abcdef` (убрать перед production) |
