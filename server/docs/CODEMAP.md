...
| `server/tickets/notification_service.py` | In-app notification recipient resolver: legacy queue/assignee/requester/watchers delivery plus executable request-template/effective registry `notification_policy` event blocks (`on_status_changed`, `on_sla_breach`, etc.) before user notification preferences |
