...
| `server/access_control/` | Server-owned RBAC catalog and resolver для нового Access Control Center: permission codes, Russian labels, risk groups, built-in role defaults, workspace derivation, `permissions_version` and effective-access calculation. |
| `server/web_api/access_handlers.py` | Typed read-only admin access endpoints `GET /api/web/admin/access/catalog`, `GET /api/web/admin/access/summary`, `GET /api/web/admin/access/effective`; питают `/app/admin/access` пользователями из `ui_users`, очередями и effective permissions без raw JSON authoring. |
