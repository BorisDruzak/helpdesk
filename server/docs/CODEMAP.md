...
| `server/web_api/` | Typed web-facing boundary для нового `webapp/`: `session_handlers.py`, `support_handlers.py`, `admin_handlers.py`, `realtime_handlers.py` и DTO-модели в `dto/*`; `session_handlers.py` обслуживает `POST /api/web/session/login`, `POST /api/web/session/logout`, `GET /api/web/session/me` через httpOnly cookie flow, `support_handlers.py` уже даёт queue/detail/action routes `GET /api/web/support/bootstrap`, `GET /api/web/support/queue`, `GET /api/web/support/tickets/{ticket_id}`, `GET /api/web/support/tickets/{ticket_id}/tools`, `POST /api/web/support/tickets/{ticket_id}/messages`, `POST /api/web/support/tickets/{ticket_id}/status`, `POST /api/web/support/tickets/{ticket_id}/tools/run`; typed detail payload теперь включает chat + `tool_call_started` / `tool_call_result` timeline entries, observer root trace metadata и device-bound tool inventory/run surface для `/app/support`, а bootstrap-ответы рекламируют observer capability links для `/app/support` и `/app/admin` |
...
| `webapp/` | Новый React/Vite foundation для внутренних workspace-экранов `/app/support` и `/app/admin`: `src/main.tsx`, `src/app/router.tsx`, `src/app/providers/query-provider.tsx`, `src/app/layouts/app-shell.tsx`, `src/features/auth/*` с русским login/session shell, а `src/features/queues/api.ts` + `src/features/queues/support-workspace.tsx` уже реализуют первый реальный support workspace slice: queue/filter/selection, ticket timeline, быстрые статусы, composer ответа и компактный snapshot устройства/операций; `playwright.config.ts`, `tests/fixtures/support_fixture_server.py` и `tests/support-workspace.spec.ts` дают локальный E2E smoke/signoff для русского `/app/support` поверх того же `server/static_pages/webapp_assets.py`, что используется сервером для выдачи built bundle |
