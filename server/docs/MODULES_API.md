...
Ticket-bound auto-install/auto-upgrade for the runner uses the same module lifecycle as any protected managed module: the server writes `device_desired_modules`, calls reconcile, and the agent receives `install_module_package`. The dependency between the parent recipe operation and the module install operation is stored in `operation_dependencies`; no recipe workflow may download or install the runner outside the module registry, preferred-version assignment and reconcile path.

Fleet rollout/canary/waves/rollback for `agent_recipe_runner` is modeled separately from ticket-bound runtime dependencies. The admin workflow persists plans, waves, targets and events in `runner_rollout_*` tables and exposes `/api/web/admin/capabilities/runner-rollout*`. Starting canary, promoting a wave or rolling back still uses `device_desired_modules` plus `modules.reconcile.reconcile_device`; it never writes module storage or sends install commands directly.
