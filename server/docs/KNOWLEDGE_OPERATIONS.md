...
Historical Knowledge audience-rule enforcement remains documented for Knowledge-owned requester/admin surfaces. It no longer extends into Helpdesk or Registry: support ticket suggestions and passport-to-Knowledge drafts are removed, the support aggregate exposes only the typed unavailable external projection, and the support audience live smoke no longer probes the deleted route. Registry quality and CSV import/export inspect Registry-owned data only; `knowledge_audience_rules` is not a supported Registry export type. External integration is pending the versioned domain port.
