...
The first backend slices add `server/knowledge/access_service.py`, `server/knowledge/audience_rules_service.py`, migration `121` for `knowledge_audience_rules`, admin-only audience-rule list/replace/preview/explain APIs under `/api/web/admin/knowledge/*`, and HTTP search/suggestion enforcement by passing Registry effective audience into `KnowledgeSearchService.search(..., effective_audience=...)`. Remaining Phase 5 work must wire the same service through direct reads, portal, support knowledge, graph/vector retrieval and RAG before authoring UI is exposed.
