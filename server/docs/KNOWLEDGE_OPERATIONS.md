...
- `/app/admin/knowledge/studio` includes `Связь с обращениями` for helpdesk context linking. The panel uses `/api/service-catalog/current` to choose service/offering/request-template context, saves or updates an existing article binding through `POST|PATCH /api/web/knowledge/items/{item_id_or_slug}/bindings`, deletes bindings through `DELETE /api/web/knowledge/items/{item_id_or_slug}/bindings/{binding_id}`, reads saved links through `GET /api/web/knowledge/items/{item_id_or_slug}/bindings`, stores selected surfaces in `metadata.surfaces`, and keeps raw binding JSON out of the normal authoring UI.
...
The first backend slices add `server/knowledge/access_service.py`, `server/knowledge/audience_rules_service.py`, migration `121` for `knowledge_audience_rules`, admin-only audience-rule list/replace/preview/explain APIs under `/api/web/admin/knowledge/*`, HTTP search/suggestion enforcement by passing Registry effective audience into `KnowledgeSearchService.search(..., effective_audience=...)`, direct requester article-detail denial before body/segment projection, requester portal home/space/tag list filtering before article summaries are projected, requester portal action denial before feedback/correction/bookmark writes, Ask/RAG/vector retrieval filtering by passing Registry effective audience into `KnowledgeRetrievalService.retrieve(..., effective_audience=...)` before ordering/rerank/citations/answer prompt construction, and support ticket knowledge-suggestions filtering that takes requester-safe P2 articles from the ticket requester audience while preserving `support_internal` runbooks from the support context. Phase 5 live anti-leak evidence is recorded by `scripts/knowledge_audience_live_smoke.py` under `artifacts/browser_live_validation/phase5-knowledge-audience-20260614145845/live-smoke-report.json`. Phase 6 exposes the first authoring selector in `/app/admin/knowledge/studio` while preserving the same requester/support/admin boundaries; browser/live-smoke evidence for commit `8d29b879` is recorded under `artifacts/browser_live_validation/phase6-knowledge-visibility-8d29b879-20260614-153908/`. Phase 8 Registry quality now reports invalid Knowledge audience-rule targets and requester-visible items whose audience rules resolve to zero known Registry users, and Registry CSV export can include `knowledge_audience_rules` without article bodies or hidden content.
...
`/app/admin/knowledge` is the Knowledge Operations Center, not an article CRUD/editor page and not a content-pack/debug console. The default screen renders health coverage plus Russian-first actionable queues from `GET /api/web/knowledge/ops/summary`: `no_audience_users`, `missing_helpdesk_binding`, `stale_article`, `indexing_failed`, `low_quality` and `zero_result_searches`. Queue items are safe action pointers with titles or redacted queries, reasons and admin URLs; the UI uses product labels such as `Статьи без аудитории`, `Статьи без связи с обращениями`, `Просроченные статьи`, `Ошибки индексации`, `Низкое качество` and `Поиски без результата`.

Each queue item must have a next action to the fixing surface: Studio article, Sections, Indexing, Search settings or Import/create article. The dashboard also provides fast transitions to dedicated workbenches with Russian hover hints. It must not expose raw content-pack JSON, hidden requester data, article bodies, chunks, `binding_id`, `job_id`, rule ids or metadata JSON as the normal workflow. Authoring, binding, policy edits, content packs, import and debug controls stay in `/app/admin/knowledge/studio` or the dedicated workbenches.
