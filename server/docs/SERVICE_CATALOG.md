...
P4 Problem Management stores `service_code`, `offering_code`, `request_type` and `reporting_category` on problem candidates, problem records, affected objects and problem analytics. Candidate detection groups repeated incidents, low CSAT, reopens, SLA breaches and failed knowledge by service/offering where available. Invalid service/offering codes are rejected when the catalog is present; legacy tickets remain bucketed as legacy/uncategorized. See [PROBLEM_MANAGEMENT.md](PROBLEM_MANAGEMENT.md).
