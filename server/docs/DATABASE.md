...
**Retained historical Knowledge tables (PR-6):** migrations `083`–`087`, `110`–`113`, `117`–`119`, `121` and `123` remain in Alembic history and their physical tables/rows remain intact, including the former content, search, audience, AI, indexing and `ticket_knowledge_links` tables. They have no active Helpdesk ORM mapping, repository, route, UI, job or configuration surface. `ticket_kb_links` also remains physical read-only history. PR-7 is external API integration only; PR-11 is the explicit acceptance-gated, forward-only schema-deletion decision. Until PR-11, do not edit old migrations, run local Knowledge schema jobs or infer that retained data is active.
