...
**Retained historical Knowledge tables (PR-6):** migrations `083`–`087`, `110`–`113`, `117`–`119`, `121` and `123` remain in Alembic history and their physical tables/rows remain intact, including the former content, search, audience, AI, indexing and `ticket_knowledge_links` tables. `TicketKbLink`, its repository and the ticket snapshot may expose `ticket_kb_links` only as a read-only historical projection: they do not mutate links, resolve local Knowledge or activate local content behavior. Sanitized `knowledge_attempts` is likewise read-only history. PR-7 is external API integration only; PR-11 is the explicit acceptance-gated, forward-only schema-deletion decision. Until PR-11, do not edit old migrations, run retired schema jobs or infer that retained data is active.
