...
**Knowledge article segmentation (migration 112):** `knowledge_segmentation_profiles`, `knowledge_article_segments`, and `knowledge_segmentation_jobs` store manual/auto/AI-ready article markup. Phase 3 implements manual segment CRUD, non-AI heading/paragraph auto segmentation jobs, admin-owned segmentation profiles, version offsets, content hashes, segment visibility, keywords, boost flags and active segment contribution to AI-off search. The same tables now support version-to-version segment revalidation/remap, stale target copies when text no longer matches, AI policy-gated draft proposals, approve/reject review, and Observer-visible `agent_runtime_audit` rows for segmentation lifecycle events. Segment index sync writes active `full_text_enabled` segments into existing `knowledge_chunks` rows with `metadata_json.source=article_segment` and `embedding_status=pending|disabled`; provider-backed embedding generation remains the Phase 4 worker concern.
...
**Knowledge audience rules (migration 121):** `knowledge_audience_rules` stores additive Registry Visibility Foundation allow rules for Knowledge spaces/items. Rules target person, department, department_tree, location, access_group, audience_group, role or service and use `status=active|disabled|archived` instead of hard deletes. `department` matches the requester's current primary department exactly; `department_tree` matches the effective department path/subtree. Rules that target `audience_group` inherit Registry audience-group expansion: `member_type=department` with `include_children=true` intentionally matches the department path including the root and is equivalent to `department_tree`. `server/knowledge/access_service.py` evaluates these rules after lifecycle/coarse visibility checks, and requester-facing paths must keep denied rule ids and hidden article metadata out of projections.

**Knowledge RAG eligibility:** `knowledge_spaces.allow_rag` is the section default for AI/RAG use, and article-level overrides live in `knowledge_items.metadata_json.ai_rag_policy` with values `inherit`, `allowed`, `disabled`, `staff_only` or `requester_safe_only`. This is an application-level gate, not a schema migration: retrieval and vector candidate services apply it after visibility/audience filtering and before rerank, citations or Ask prompt construction. Requester/agent/public responses must not expose excluded article title, slug, body, snippet, chunks or metadata.
...
**registry_audience_groups / registry_audience_group_members:** content/service/notification targeting groups for Registry Visibility Foundation. They are separate from `access_groups`: membership can target Knowledge or service audiences but never grants RBAC permissions. Members can reference person, department, department_tree, location, access_group, role or service; `department` with `include_children=true` intentionally follows the same subtree/path expansion as `department_tree`, while `department` with `include_children=false` remains direct-current-department targeting. Archive uses `status`, not hard delete. Admin CRUD and preview expansion are under `/api/web/admin/registry/audience-groups*`.
