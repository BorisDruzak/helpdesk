...
**Knowledge article segmentation (migration 112):** `knowledge_segmentation_profiles`, `knowledge_article_segments`, and `knowledge_segmentation_jobs` store manual/auto/AI-ready article markup. Phase 3A implements manual segment CRUD, non-AI heading/paragraph auto segmentation jobs, admin-owned segmentation profiles, version offsets, content hashes, segment visibility, keywords, boost flags and active segment contribution to AI-off search. AI proposal review, embedding sync and stale remap automation remain later Knowledge vNext slices.
