...
Integrity checkers that use bounded reads must return complete/incomplete coverage through `ObserverIntegrityCheckResult`; use `LIMIT + 1`, count/cursor pagination or another full-coverage proof, not the number of generated violations after Python-side filtering. Incomplete, failed, timed-out or legacy/plain-list checker results must not resolve existing events for their source. The runner records each checker in `observer_integrity_check_runs` with status, duration, scanned/window metadata, generated/active/suppressed/resolved counts and redacted error details; scan API responses must expose the same `checks` report for live evidence. Repeated observations must preserve operator `acknowledged` state until a checker report with `status=passed` and `complete=true` proves the condition disappeared. Missing requester-create coverage must use the strict success predicate, not any trace with matching ticket/source text.
