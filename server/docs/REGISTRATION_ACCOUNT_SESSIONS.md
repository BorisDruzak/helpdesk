...
- Manual code entry is handled by web-authenticated `POST /api/web/registry/browser-pairings/lookup` and `/app/device/pair`. The lookup accepts web-session roles `admin`, `support` and `user`, is rate-limited, accepts the short-lived pairing code in the JSON body, rejects expired/consumed/superseded rows, and returns only `pairing_id`, `purpose`, `expires_at` and `next_url`.
- Browser confirmation is handled by web-authenticated `/api/web/registry/browser-pairings/{pairing_id}` plus `/login/confirm` or `/registration/confirm`; browser pages are `/app/device/pair`, `/app/device/login` and `/app/device/register`. The web endpoints accept web-session roles `admin`, `support` and `user`; login confirmation still resolves the actor to a registry person and must match an active binding for the pairing device.
- Login confirmation resolves the web user to a registry person and requires an active primary/shared/responsible binding for the pairing device. Registration confirmation creates or updates the registration claim for the pairing device through `RegistrationService` and is not blocked by an incomplete requester profile.
- By default, the first non-conflicting browser registration confirmation is auto-approved into an active device binding. If `registration.require_admin_confirmation=true` and `registration.auto_approve_first_binding=false`, the same flow remains pending for administrator review.
- When registration policies require existing Registry values, `/app/device/register` uses `/api/registry/options` pickers and sends only selected `department_id` / `location_id` in the `/registration/confirm` JSON body. `/api/registry/options` is the narrow non-`/api/web` registry route that accepts the same web-session cookie for this browser page; do not broaden cookie auth to all agent registry endpoints. The server still validates those ids through `RegistrationService`; the browser cannot submit raw account-session tokens or override person/binding fields.
- Browser confirmation marks the pairing as confirmed but does not return an account `session_token` to the browser UI.
- The agent pickup creates/returns the account `session_token` once for login and marks the pairing `consumed`; later pickups return session metadata without the token. Registration pairing pickup keeps the row `confirmed` while the claim is still `pending_admin_review`, `conflict` or otherwise lacks a deliverable confirmed-binding session. It marks `consumed` only when pickup can create and return the account session token after approval.
