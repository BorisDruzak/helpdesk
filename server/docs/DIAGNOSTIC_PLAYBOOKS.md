...
Update note 2026-04-28/2026-05-04 ticket launch: support ticket detail now has typed playbook launch routes. `GET /api/web/support/tickets/{ticket_id}/playbooks` returns published playbook versions with required tools, missing tools, missing required params, readiness for the ticket device, and recent ticket playbook runs with step errors. `POST /api/web/support/tickets/{ticket_id}/playbooks/run` starts the selected version through `playbook_engine.start_run` with `trigger_type=support_ticket` and a ticket-bound context, but blocks `PLAYBOOK_PREFLIGHT_BLOCKED` before enqueue if the playbook is not runnable.
