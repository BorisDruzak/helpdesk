...
* Scenario B is not fully closed until the mapped safe error is rerun on the canonical stand with browser-visible evidence. Backend API may continue to return stable technical strings for contract/debugging; the product UI must not render those strings to the requester.
* Next Scenario B rerun can be narrower: deploy the UI mapping, reuse the strict picker setup, invoke the same real-agent browser-registration action, drive a browser-equivalent invalid POST with `Origin`/`Referer`, and focus on the Russian-safe visible error plus no-side-effect DB assertions. Do not rerun the full Scenario A approval/ticket path for this negative proof.
* Before closing Scenario B, distinguish the proof layers explicitly. A direct invalid `registration/confirm` POST proves backend/API validation and DB no-side-effects, but it does not by itself prove the requester page renders the mapped error. Browser-visible closure requires the actual `/app/device/register` page to receive the failed confirm response through its UI flow or through a controlled browser interception/test harness that exercises the page error path; if Browser/CDP interception is unavailable, record the tooling limitation and keep Scenario B as `needs-clean-rerun`.
* Before interpreting the next browser-visible error, verify the deployed stand SHA or bundled asset includes the local mapping fix. If the stand is still on pre-fix `4ca9a11` behavior, raw `department_id not found` is expected and must not be counted as a post-fix regression.
* Fresh-run conditions for the next Scenario B attempt: create a new run marker and pairing after browser auth is confirmed; perform the invalid submission before pairing TTL expiry; keep both policy modes at `required_existing` only for the run; capture browser DOM/screenshot or bounded DOM text, sanitized API response, sanitized DB snapshot and policy restore evidence; stop remote services and the isolated local agent at cleanup unless the user explicitly asks to keep them running.
* Scenario C should not start until Scenario B status is either closed with browser-visible evidence or explicitly deferred by the user. If Scenario C must proceed first, record the deferral in this block and add contamination checks so later Knowledge/Phase 7 work does not treat Phase 4 negative registration coverage as complete.
