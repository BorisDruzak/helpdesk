...
- 2026-06-17 active follow-up: fix the browser path opened from the agent so a user can create a web account before profile setup/device-link confirmation, preserving the original device-link `next` path across register/login/profile setup; add agent startup self-update so a launcher-run agent automatically requests the current recommended build once after a successful handshake when no update is already pending. Required gates: focused red/green frontend and agent tests, real browser evidence on `https://192.168.100.17:9443`, Windows release build, upload as the priority stable build, local launcher canary proving the old local agent updates itself, then docs/handoff update.
