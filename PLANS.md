...
- **Latest Observer canary blocker:** the first `run_observer_canary_suite.py --source-coverage-only` attempt for `requester_support_admin_session_switch` did not reach Observer coverage because the script still posts to legacy `/api/ui_login`; stand correctly returns HTTP 410 `LEGACY_AUTH_DISABLED` and points to `/api/web/session/login`. This is tooling/auth drift in the canary path and must be fixed before the Observer canary can satisfy the scenario manifest or move P0 toward `[x]`.
