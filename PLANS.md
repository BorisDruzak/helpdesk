...
- [ ] `QG-001` Добавить `scripts_pytest_no_db`.
- [ ] `QG-002` Добавить strict test inventory audit.
...
- [ ] `QG-006` Запретить release-pass на shared DB.
...
- [ ] `OBS-103` Сохранить acknowledged state при повторном scan.
- [ ] `OBS-104` Добавить per-check execution report и failure isolation.
- [ ] `OBS-105` Ужесточить successful create trace predicate.
- [ ] `OBS-106` Characterize и исправить web execution trace identity.
- [ ] `DB-101` Создать schema-to-cleanup audit.
- [ ] `DB-102` Классифицировать все текущие tables.
- [ ] `DB-103` Закрыть `change_*` и новый Knowledge cleanup drift.
...
- [ ] `LIVE-301` Ввести evidence manifest v2.
- [ ] `LIVE-302` Реализовать validator.
...
1. `server/tests/test_observer_integrity_scan_scope.py` — reproducer `limit + 1`.
2. Исправление resolution API без изменения доменной логики.
3. `scripts/test_db_cleanup_schema.py` + dynamic audit.
4. Добавление отсутствующих DB classifications/profiles.
5. `scripts_pytest_no_db` в CI и тест состава шагов.
6. Tests acknowledgement lifecycle и failed-checker behavior.
7. Tests web trace repeated/retry/error→success.
8. Strict successful create-trace predicate.
9. Evidence manifest v2 validator.
