...
* Scenario B is not fully closed until the user-visible error contract is decided or fixed. Current backend/frontend behavior would surface raw English `department_id not found` if the product UI rendered the server response. If Russian-safe UX is still a pass condition, add a focused regression/fix that maps invalid department/location ids to a Russian safe message before claiming Scenario B complete.
* Next Scenario B rerun can be narrower: reuse the strict picker setup, invoke the same real-agent browser-registration action, drive a browser-equivalent invalid POST with `Origin`/`Referer`, and focus on the Russian-safe visible error plus no-side-effect DB assertions. Do not rerun the full Scenario A approval/ticket path for this negative proof.
