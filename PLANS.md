...
- начат и практически доведён `Task 8`: новый typed realtime bridge уже живёт в `server/web_api/realtime_handlers.py` + `webapp/src/shared/realtime/*`, bootstrap `GET /api/web/realtime/bootstrap` рекламирует `ws_ui` transport contract и session-cookie auth, `server/websocket/ui_handler.py` поддерживает `ui_hello` через httpOnly cookie и `skip_catchup`, а `/app/support` и `/app/admin` уже получают live invalidation для ticket/device slices без raw websocket-логики в feature-коде React.
  ...
  - закрыт parity/cutover prep для нового shell: `/api/web/session/me` теперь несёт `default_workspace` и `available_workspaces`, `/app` стал role-aware entrypoint, `/app/admin` закрывается access-gate для support-only сессии, а legacy `/login`, `/support`, `/admin` готовы к переключению через `WEBAPP_CUTOVER_*` с явным escape `?legacy=1`.
  ...
  - для живого signoff нового shell появился canonical helper `webapp/scripts/remote-browser-signoff.mjs` и команда `pnpm --dir webapp run check:remote -- --base-url http://192.168.100.17:8666`: она логинится в live `/app/*`, подтверждает `/app` -> default workspace redirect, session payload, русскую локаль, deep admin slices и отсутствие browser console/page errors без ad-hoc one-liner'ов.
  - operational cutover layer для `Task 10` теперь материализован в `server/static_pages/cutover.py` и `scripts/check_webapp_cutover.py`: route switch активируется только при наличии built bundle, а `/support` и `/admin` дополнительно требуют включённый login cutover; при несобранном bundle или нарушенных prerequisites старые маршруты остаются на legacy shell, а `?legacy=1` по-прежнему является жёстким escape.
  - live signoff helper `webapp/scripts/remote-browser-signoff.mjs` теперь кроме `/app`, `/app/admin` и `/app/support` валидирует raw redirects `/login`, `/admin`, `/support` в `/app/*` и raw legacy escape `/login?legacy=1`, `/admin?legacy=1`, `/support?legacy=1`.
  - targeted legacy cleanup начат без premature delete: `server/ticket.html` больше не прибивает пользователя к `_shell=...` и использует логический `/admin`, а полный снос legacy `login/admin/support` assets отложен до явного завершения migration window.
