...
- [~] **P0 full live behavior pack**: automation prerequisite improved; browser dry-run now covers all 14 browser scenarios across `requester,support,admin,reports`, agent-operation dry-run covers 2 scenarios, and exact-context release summary for `66be2997b551027a0776d9587ac433117ab451b6`/`stand` is generated as `status=blocked` with all 17 scenarios missing. Still open until the full `critical_behavior_v1.json` run on one frozen commit/environment produces 17 passing `pc_client.live_evidence.v2` manifests.
