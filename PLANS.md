...
2026-06-13 Knowledge Studio/Metadata audit fixes:

* `/app/admin/knowledge/studio` шаг `2. Метаданные` перестроен под сценарий автора: сначала `Куда попадёт статья` с пространством, типом и видимостью; `slug`, tags, owner и reviewer вынесены в `Advanced / служебные поля статьи`.
* В `ArticleMetadataPanel` multi-select свойства больше не редактируются через raw textarea: значения выбираются чекбоксами, а сохранение показывает явный success/error state.
* Правила применимости статьи и глобальной metadata page используют общий редактор: service/offering берутся из `/api/service-catalog/current`, правило можно добавить, отредактировать, удалить и затем сохранить.
* `/app/admin/knowledge/metadata` свойства получили chip-editor для allowed values и чекбоксы для типов материалов вместо многострочных служебных полей; сохранение показывает `Свойство знаний сохранено`.
* Проверено локально: `pnpm --dir webapp exec vitest run src/features/knowledge/article-metadata-panel.test.tsx --reporter=dot`; `pnpm --dir webapp exec vitest run src/features/knowledge/metadata-page.test.tsx --reporter=dot`; `pnpm --dir webapp exec vitest run src/pages/admin/knowledge-studio-page.test.tsx --reporter=dot`; `pnpm --dir webapp exec tsc --noEmit --pretty false`; `pnpm --dir webapp build`; `git diff --check -- ...`.
* Browser/live evidence обновлено после quick deploy на стенд: `/app/admin/knowledge/studio` и `/app/admin/knowledge/metadata` проверены на 1366×768 и 1920×1080; horizontal body scroll отсутствует, `Создать версию` видна без прокрутки в Studio, `Куда попадёт статья`, `Advanced / служебные поля статьи`, `Новое разрешённое значение`, `Сервис или услуга` и hint выбора service/offering видимы; console warn/error = 0.
* Локальные/untracked evidence files: `artifacts/knowledge-metadata-audit-20260613/knowledge-studio-metadata-audit-1366x768.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-studio-metadata-audit-1920x1080.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-metadata-properties-audit-1366x768.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-metadata-properties-audit-1920x1080.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-metadata-applicability-audit-1366x768.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-metadata-applicability-audit-1920x1080.png`, `artifacts/knowledge-metadata-audit-20260613/knowledge-metadata-audit-evidence.json`.
