...
Post-review maintenance in progress, 2026-06-14: tightening the closed Phase 5/8 foundation by resolving active Registry audience groups into `EffectiveIdentity`/`EffectiveAudience`, making `department` exact and `department_tree` path-based, extending admin preview to `subject_type=space`, updating stale docs/plan wording, and rerunning local plus live audience-group smoke verification.
