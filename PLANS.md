...
- **Current implementation revision for P0/CI isolation work:** `adf73267c970200d0ab29b25e8419c0299ddb8c5`.
...
- **Latest CI unblock evidence:** full CI on `69191f0e` reached `server_pytest_db_agent_runtime` after passing workspace verify, webapp bundle/unit/fixture E2E, audits/smokes, no-DB, migration schema, knowledge DB, tickets DB and observer diagnostics DB layers; `server_pytest_db_agent_runtime` blockers were fixed by DB cleanup/test-isolation commit `adf73267`, and the focused layer now passes (`127 passed, 123 deselected in 471.20s`).
...
- [~] **P0 Observer scan completeness**: **статус 2026-06-24: code-side fix и focused DB-proof закрыты, а найденные full-CI test-isolation blockers закрыты на `adf73267`; пункт остается `[~]`, не `[x]`, пока plain full CI/release/live evidence не покрывают финальный HEAD.** Fail-closed path реализован: все top-level checkers возвращают `ObserverIntegrityCheckResult`, bounded scans используют `LIMIT + 1` windows, `runtime_presence` возвращает incomplete при отсутствии state, неизвестная `source_complete` больше не считается complete по умолчанию. Свежая DB-улика: весь `server/tests/test_observer_integrity_scan_completeness.py` прошел в isolated PostgreSQL без shared fallback (`4 passed in 473.93s`), покрывая 201 operation, 301 web cabinet, 501 runtime и DB-сценарий "incomplete scan не resolve, complete scan resolve только исчезнувшую visible finding, не outside-window finding". `server/tests/test_observer_integrity_scan_scope.py` также прошел (`8 passed in 0.31s`). Дополнительная CI-улика: `server_pytest_db_agent_runtime` после cleanup/test-isolation правок прошел (`127 passed, 123 deselected in 471.20s`). Недостающее для перевода в `[x]`: актуальный plain `python scripts/run_ci_suite.py`, `python scripts/release_candidate_preflight.py --commit <HEAD>` и full live behavior pack на одном frozen commit/environment без shared DB fallback.
