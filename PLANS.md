...
* Required next checkpoint before Phase 5/Phase 7: deploy the support detail projection fix, reopen `T-000694` or create a fresh ticket B, verify same-browser `/api/web/support/tickets/{ticket_id}` and `/workspace` expose the account-session fields, and capture a passing support UI warning artifact showing `Обращение создано с другого аккаунта на зарегистрированном устройстве.`. Do not treat DB-only warning evidence as a Scenario C pass because the operator-facing support warning is an explicit acceptance condition.
