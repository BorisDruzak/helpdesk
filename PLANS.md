...
* Support-browser warning proof is blocked by a product/API gap, not by missing browser setup. `support_ticket_b_missing_warning.safe.json` and `support_ticket_b_missing_warning.png` show `/app/tickets/f3a93df4-e82d-4fb7-a5c2-44aaf0b38c0a` rendered ticket `T-000694` with the other-account marker, but the expected warning text was absent. Same-browser fetches of `/api/web/support/tickets/{ticket_id}` and `/api/web/support/tickets/{ticket_id}/workspace` returned HTTP 200 for `T-000694` but `requester_account_warning`, `requester_account_mode`, `requester_account_session_id` and `requester_account_context` were all null/empty.
* Likely fix target: `server/web_api/support_handlers.py::_build_support_detail_payload` calls `_ticket_payload(...)`, and `_ticket_payload`/`server/app/api/serializers.py` already know `requester_account_warning`, `requester_account_mode`, `requester_account_session_id` and `requester_account_context`; the `SupportTicketDetail(...)` construction currently does not pass those fields through. Add backend support handler coverage plus frontend mapper/UI coverage before rerunning the browser warning proof.
* Required next checkpoint before Phase 5/Phase 7: add a focused regression test that support detail/workspace include account-session fields for verified other-account tickets, keep `webapp/src/features/queues/support-workspace-mappers.test.ts` asserting the warning maps to `viewModel.right.context.requester.accountWarning`, deploy the fix, reopen `T-000694` or create a fresh ticket B, and capture a passing `support_ticket_b_warning.safe.json`/`.png`. Do not treat DB-only warning evidence as a Scenario C pass because the operator-facing support warning is an explicit acceptance condition.
...
* Artifact hygiene remains open: `ticket_b_create.safe.json` still has a misleading safe suffix and contains a raw public access code plus declared-account contact fields. Do not stage/share it; replace with a redacted create summary if a clean handoff bundle is needed.
