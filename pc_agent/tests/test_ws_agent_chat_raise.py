...
@pytest.mark.asyncio
async def test_chat_raise_returns_server_error_details(tmp_path):
    agent = WSAgent(data_root=tmp_path, install_root=tmp_path / "install")
    agent.device_id = "device-test-1"
    agent._agent_ws = _WsStub()
        ...
        pending.set_result(
            {
                "payload": {
                    "status": "error",
                    "error": {
                        "code": "TICKET_CREATE_UNAVAILABLE",
                        "message": "Ticket creation is unavailable for chat_raise",
                    },
                    "data": {"observations": {}},
                }
            }
    ...
    assert result == {
        "ok": False,
        "error_code": "TICKET_CREATE_UNAVAILABLE",
        "error": "Ticket creation is unavailable for chat_raise",
    }
