...
@pytest.mark.asyncio
async def test_pre_canceled_background_dispatch_does_not_execute_tool(tmp_path):
    db_path = tmp_path / "storage.db"
    DatabaseManager._instance = None
    db = DatabaseManager(str(db_path))
    await db.init_db()
    ...
    agent = WSAgent(data_root=tmp_path)
    agent.db_manager = db
    command_id = "00000000-0000-0000-0000-00000000c003"
    cached_payload = {
        "status": "canceled",
        "data": {"observations": {"cancel_status": "canceled", "target_operation_id": command_id}},
        "error": {"code": "OPERATION_CANCELED", "message": "Command was canceled before execution"},
        "meta": {"request_id": command_id},
    }
    await db.mark_command_started(command_id, owner_instance_id="test-session")
    await db.mark_command_seen(
    ...
    async def should_not_execute(*_args, **_kwargs):
        raise AssertionError("pre-canceled command must not execute")
