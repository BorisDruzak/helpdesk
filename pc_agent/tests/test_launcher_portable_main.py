...
def test_launcher_stops_after_repeated_immediate_crash_without_rollback(monkeypatch, tmp_path):
    ...
    def _fake_popen(argv, **kwargs):
        launches.append(Path(argv[0]).parent.name)
        if len(launches) > launcher_portable_main.IMMEDIATE_CRASH_RETRY_LIMIT:
            raise AssertionError("launcher restarted after terminal startup crash")
        stdout = kwargs.get("stdout")
        if stdout is not None:
            stdout.write(b"ImportError: DLL load failed while importing QtWidgets\n")
            stdout.flush()
        return _FakeProc(next(exit_codes))
    ...
    assert failed_launch["attempts"] == launcher_portable_main.IMMEDIATE_CRASH_RETRY_LIMIT
    assert "rollback is unavailable" in failed_launch["message"]
    assert shown_errors == [
        (
            "Maria Agent failed to start",
            failed_launch["message"],
        )
    ]
