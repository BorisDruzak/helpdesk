...
- `3.1.76` supersedes `3.1.75` for authenticated GUI navigation and visible update progress: the main sidebar exposes `Веб кабинет` after login, startup/manual self-update surfaces the `applying` lifecycle before exit code 42, and the GUI shows an update-in-progress dialog while the agent prepares to restart. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.75` artifacts.
- `3.1.75` supersedes `3.1.74` for the web-cabinet GUI entrypoint: the account gate exposes a direct `Веб кабинет` button, visible fallback links to the cabinet/login/password-reset request routes, and uses the same public `/app/*` origin normalization as browser pairing. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.74` artifacts.
- `3.1.73` supersedes `3.1.72` for web-profile/account-card sync after browser registration: when the requester completes the web profile after the agent already created a confirmed-binding session, the GUI refresh path replaces stale local login/email display fields with fresh `account-state` person fields without logout. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.72` artifacts.
- `3.1.72` supersedes `3.1.71` for delayed browser-registration confirmation recovery: if the browser-confirmed binding is discovered later through `account-state` instead of the immediate pairing pickup response, the GUI auto-creates the confirmed-binding account session and opens the ticket workspace instead of leaving the agent on the account gate. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.71` artifacts.
- `3.1.71` supersedes `3.1.70` for the browser-registration handoff: when the server auto-approves the first non-conflicting binding, the GUI creates a confirmed-binding account session and opens the ticket workspace instead of staying on the account gate. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.70` artifacts.
- `3.1.70` supersedes `3.1.69` for the no-token GUI authorization dialog: archived devices now show a clear `DEVICE_ARCHIVED` explanation instead of telling the user that an admin approval request is waiting in the queue. Publish Windows and Linux builds as new upload/rollout targets; do not overwrite `3.1.69` artifacts.
- `3.1.68` supersedes `3.1.67` for the browser registration handoff and startup self-update refresh: web registration/login/profile setup preserve the original device-link `next` path, profile setup exposes a safe "Создать аккаунт" CTA for the account-first flow, `ws_agent.py` requests the server-recommended build once after a successful startup handshake when no `pending_update.json` exists, and the server handshake can enqueue `agent_handshake_auto_update` for older installed agents that do not yet contain the client-side startup trigger. Publish it as a new upload/rollout target; do not overwrite `3.1.67` artifacts.
...
- GUI update UX should expose the request lifecycle explicitly: `requesting`, `requested`, `pending_restart`, `applying`/`restarting`, then handshake-confirmed terminal success/failure.
...
3. browser check на `https://example.test:9443/admin`
