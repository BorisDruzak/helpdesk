...
Скриншоты экрана и запись экрана (видео). Доступен как встроенный модуль (`modules/impl/screen.py`) и как **пакет для загрузки на сервер** (см. раздел «Пакеты модулей для сервера и агентов»). Начиная с актуальной конфигурации агент всегда нормализует `enabled_modules` так, чтобы builtin-модули `system` и `screen` были включены даже при старом или урезанном `settings.yaml`.
...
Модули из `modules/impl/` (screen, input и др.) могут также поставляться как ZIP-пакеты для серверной раздачи. Для `screen` это теперь опциональный сценарий: основной путь работы — builtin-модуль внутри агента, без отдельной установки. ZIP-пакет нужен только для совместимости со старыми агентами или если требуется именно server-managed package workflow.
...
- Для актуальных агентов отдельная установка `screen` не требуется: модуль уже встроен и принудительно включается в базовую конфигурацию.
- После установки пакета на старый агент ошибка «Экземпляр модуля "screen" не найден в loaded_modules» устраняется: модуль загружается из `modules_store` при старте и при установке.
