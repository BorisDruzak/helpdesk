...
| `pc_agent/ui_gui/account_gate.py` | Non-modal first-screen account gate: displays server account-state, lets the user enter as confirmed registered account, continue/confirm pending registration or create an explicit local other-account session; if no confirmed account is visible, the gate still offers registration and other-account login as local fallback actions instead of leaving only refresh/settings; other-account login never calls registration endpoints and every created ticket carries `requester_account.created_from_other_account=true` |
