...
- Windows stable agent `3.1.46` supersedes `3.1.45` for Remote Assist clipboard/control hotfixes: `clipboard_enable` / `clipboard_disable` are routed to the clipboard bridge, sanitized data-channel decisions are logged, and Windows `SendInput` attaches to the foreground input queue before injecting input. `3.1.45` carried the quality/control/clipboard slice: requested capture quality is passed into the WebRTC screen track, approved `interactive_control` sessions auto-enable control without a second consent prompt, drag/wheel/shortcut input is supported, and text clipboard auto-sync runs only when the initial session capability and server policy allow it.
