...
| `pc_agent/ui_gui/window_chrome.py` | Кастомная frameless-шапка окна для Windows/Linux: логотип/заголовок, minimize/maximize/close, ручной drag/resize без Qt native `startSystemMove()` / `startSystemResize()`, чтобы на Windows не появлялись `_q_titlebar` helper windows в Taskbar/Alt-Tab |
...
- Windows stable agent `3.1.34` carries the Remote Assist WebRTC packaging hotfix: `pyinstaller_agent_win*.spec` collect `aiortc`/ICE/media hidden imports, and `main_window.py` reports WebRTC startup failures through `/remote-assist/{session_id}/fail` so the support viewer and ticket audit leave the waiting state. Windows stable `3.1.33` remains the prior Remote Assist runtime fix set for command handling and retained/topmost consent dialogs.
- WebRTC/capture/runtime lives under `pc_agent/remote_assist/`: `thread.py` owns a dedicated `QThread` + asyncio loop, `webrtc_client.py` owns aiortc signaling/peer connection and the `control` data channel, `screen_track.py` captures one primary monitor through `mss`, and `input_controller.py` validates interactive-control mouse/keyboard messages before Windows input injection. Control is inert unless the approved session mode is `interactive_control` and the operator sends `control_enable`.
