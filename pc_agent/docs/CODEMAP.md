...
- Windows stable agent `3.1.35` carries the Remote Assist interactive-control test release: Windows mouse/keyboard injection uses `SendInput` instead of legacy `mouse_event`/`keybd_event`, and Linux uses the `pynput` backend when policy enables `interactive_control`. Windows stable `3.1.34` remains the prior WebRTC packaging hotfix for bundled `aiortc`/ICE/media dependencies and startup-failure reporting.
