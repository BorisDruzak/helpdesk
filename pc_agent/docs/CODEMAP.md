...
2026-05-23: `pc_agent/ui_gui/dynamic_form_widget.py` is the reusable schema/field renderer for both ticket creation and agent user registration. `chat_panel.py` keeps compatibility aliases for `TicketDynamicFieldsWidget`, while `main_window.py` loads `GET /api/registry/agent/registration-form` and submits/confirm registration through the existing registration endpoints instead of the ticket create flow. Account login is a local requester session, not password auth and not a replacement for the machine token: `pc_agent/ui_gui/account_gate.py` is the first non-modal page, settings stay accessible before login, and ticket list/create/detail are gated until `AccountSessionManager` has a valid session from confirmed binding, pending registration or explicit other-account login. Before login, `main_window.py` enters account entry mode: it hides the main sidebar/footer shell, shows only the account gate, the separate registration entry page or settings, gives entry settings an explicit back action, and opens the full workspace only after confirmed/pending/other-account session selection.
...
| `pc_agent/ui_gui/account_gate.py` | Non-modal first-screen account gate: displays verified server account-state, lets the user enter as confirmed registered account, continue/confirm pending registration or create an explicit local other-account session; if a verified account-state has no confirmed account, the gate still offers registration and other-account login as fallback actions, but unknown/error states show only refresh/settings until account-state succeeds; other-account login never calls registration endpoints and every created ticket carries `requester_account.created_from_other_account=true` |
