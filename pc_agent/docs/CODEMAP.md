...
- WebRTC/capture/runtime lives under `pc_agent/remote_assist/`: `thread.py` owns a dedicated `QThread` + asyncio loop, `webrtc_client.py` owns aiortc signaling/peer connection and the `control` data channel, `tls.py` prepares the HTTPS/WSS trust context for stand/prod certificates, `screen_track.py` captures one primary monitor through `mss`, `input_controller.py` validates interactive-control mouse/keyboard messages before dispatching to the platform input backend (`SendInput` on Windows, `pynput` on Linux), `elevated_helper.py` owns the Windows UAC helper/proxy for `elevated_admin`, and `clipboard.py` owns text clipboard auto-sync for sessions that requested and received the clipboard capability. Control is active only when the approved session mode is `interactive_control` or `elevated_admin`; the viewer auto-enables it on data-channel open and can still pause/resume locally.
- View-only remains the default and all capture/WebRTC/input/clipboard work must stay outside the GUI thread. No file transfer, audio, recording, UAC bypass, or hidden unattended access is active unless a later policy-backed layer explicitly implements and verifies it.
