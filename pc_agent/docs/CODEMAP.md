...
| `pc_agent/ui_gui/server_api.py` | Обращение к серверу из GUI, создание обращений через `/api/tickets/create` с legacy `form_key` и first-class `request_template_key`, отправка `reply_to`/message metadata, вызов `mark_ticket_read()` и ticket history API с `get_ticket(..., since_event_id=..., before_event_id=..., limit=...)`; `sync_registry_profile()` отправляет локальный профиль инициатора в `/api/registry/profile` для auto-link человека, ПК и здания/кабинета |
