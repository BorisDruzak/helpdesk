...
def _run(cmd: list[str], cwd: Path) -> None:
    print(f"[build_windows_release_v2] RUN: {' '.join(cmd)}", flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Windows quiet release for pc_agent")
    parser.add_argument("--version", type=str, default=None, help="Override AGENT_VERSION")
    parser.add_argument("--channel", type=str, default="stable", help="Release channel")
    parser.add_argument("--target", type=str, default="windows_amd64", help="Server target value")
    parser.add_argument("--clean", action="store_true", help="Clean build/dist first")
    ...
    print(f"[build_windows_release_v2] Version: {version}")
    print(f"[build_windows_release_v2] Install root: {install_root}")
    print(f"[build_windows_release_v2] Artifact: {artifact_zip}")
    return 0
