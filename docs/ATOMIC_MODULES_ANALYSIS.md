...
- **Единый шаблон на сервере:** один шаблон костяка (как в п. 3.3) для генерации module.py из «только кода функции» + метаданных (имя модуля, инструмента, описание). Так уменьшается дублирование и ошибки.
- **Проверка до сохранения:** как сейчас: preflight ZIP (manifest, entrypoint) + smoke_check_module (загрузка, register, list_tools). Не сохранять и не ставить в очередь установку, пока smoke не прошёл — тогда ошибки видны сразу в веб-форме.
- **Понятные сообщения об ошибках:** preflight_errors и stderr smoke_check возвращать пользователю (без внутренних путей при необходимости), чтобы было ясно, что править в коде.
