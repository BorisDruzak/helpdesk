...
| Service Catalog process layer | `server/tickets/service_catalog_*`, `server/app/repos/service_catalog_repo.py`, `server/web_api/service_catalog_handlers.py`, `scripts/seed_service_catalog.py`, `webapp/src/features/service-catalog/*`, `webapp/src/features/request-template-studio/*`, `webapp/src/pages/admin/request-template-studio-page.tsx`, `webapp/src/pages/help/index.tsx`, `pc_agent/ui_gui/chat_panel.py`, `pc_agent/ui_gui/server_api.py` | Requester-facing services and offerings, fallback `other.unknown`, publication gates, requester-safe preview, catalog-aware create/preview, service/offering policy inheritance and reporting dimensions, guided request-template studio workflow over existing catalog/registry/policy-health APIs | `/api/web/admin/service-catalog*`, `/api/web/admin/helpdesk-model/*`, `/api/web/admin/helpdesk/policy-health*`, `/api/service-catalog/current`, `POST /api/service-catalog/preview`, create payload fields `service_code`/`offering_code`, `custom_fields.service_catalog`, ticket reporting columns | service catalog tests, ticket create regression tests, webapp build, agent GUI helper/API tests, browser signoff for `/app/admin/service-catalog`, `/app/admin/request-template-studio` and requester help flow |
