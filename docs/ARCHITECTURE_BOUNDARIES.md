...
| Service Catalog process layer | `server/tickets/service_catalog_*`, `server/app/repos/service_catalog_repo.py`, `server/web_api/service_catalog_handlers.py`, `scripts/seed_service_catalog.py`, `webapp/src/features/service-catalog/*`, `webapp/src/features/request-template-studio/*`, `webapp/src/pages/admin/request-template-studio-page.tsx`, `webapp/src/pages/help/index.tsx`, `pc_agent/ui_gui/chat_panel.py`, `pc_agent/ui_gui/server_api.py` | Requester-facing services and offerings, fallback `other.unknown`, publication gates, requester-safe preview, catalog-aware create/preview, service/offering policy inheritance and reporting dimensions. Request Template Studio orchestrates the primary configuration UX over Service Catalog, Forms Builder registry and Policy Health; those systems remain the source of truth and expert editors. | `/api/web/admin/service-catalog*`, `/api/web/admin/helpdesk-model/*`, `/api/web/admin/helpdesk/policy-health*`, `/api/service-catalog/current`, `POST /api/service-catalog/preview`, create payload fields `service_code`/`offering_code`, `custom_fields.service_catalog`, ticket reporting columns | service catalog tests, ticket create regression tests, webapp build, agent GUI helper/API tests, browser signoff for `/app/admin/service-catalog`, `/app/admin/request-template-studio` and requester help flow |
| Knowledge Platform | `server/knowledge/*`, `server/app/repos/knowledge_repo.py`, `server/web_api/knowledge_handlers.py`, `server/app/db/migrations/versions/*knowledge*`, `server/docs/KNOWLEDGE_PLATFORM.md`, `server/docs/KNOWLEDGE_OPERATIONS.md`, `scripts/seed_knowledge_content.py`, `scripts/validate_knowledge_pack_bindings.py`, `scripts/repair_knowledge_pack_bindings.py`, `content_packs/knowledge/*`, `webapp/src/features/knowledge/*`, `webapp/src/pages/help/index.tsx`, `webapp/src/pages/knowledge/index.tsx`, `webapp/src/pages/admin/knowledge-page.tsx`, `pc_agent/ui_gui/server_api.py`, `pc_agent/ui_gui/chat_panel.py` | Universal knowledge spaces/items/versions/chunks, search/suggestions, ACL visibility, canonical Service Catalog content-pack bindings, binding repair, graph, ingestion, feedback/deflection metrics, passport-to-draft, content packs, templates/lint, review tasks, quality/gap operations, search analytics, rollout policies and support knowledge panel integration | `/api/knowledge/search|suggest|feedback`, `/api/web/knowledge/*`, selected-version publish, content-pack apply/retire, binding validator/repair commands, review-task/quality/gap/search-analytics/rollout APIs, effective rollout preview, `knowledge_attempts` create payload extension, `ticket_knowledge_links`, existing `/api/tickets/{id}/kb_links` compatibility | knowledge contract/repo/visibility/search/suggestion/feedback/graph/ingestion/passport/API/metrics/publish/DB-constraint/content-pack/binding-repair/templates/lint/review/quality/gap/search-analytics/operations tests, P0/P1 regressions, webapp build, agent tests, browser signoff for `/app/admin/knowledge`, `/app/knowledge`, `/app/help`, `/app/tickets/:ticketId` |
...
| Tech Panel readiness surface | `server/tech/snapshot.py`, `server/tech/locator.py`, `server/tech/handlers.py`, `server/auth/middleware.py`, `scripts/business_smoke.py`, `scripts/write_restore_drill_marker.py`, `scripts/release_server_to_remote.py`, `webapp/src/pages/admin/tech-page.tsx`, `webapp/src/features/tech/tech-panel-api.ts` | Read-only pilot readiness, quick localization for ticket/device/hostname/operation/trace/log context, security/auth/session/TLS policy visibility, query-token attempt telemetry without token values, runtime health including inventory duplicate-task signal, PostgreSQL/migrations/backup/restore markers, agents including below-baseline device lists, operations, logs, release/smoke and safe links. Marker scripts write readiness evidence; the browser UI only reads it. | `/api/web/admin/tech/snapshot`, `/api/web/admin/tech/locate` and `/app/admin/tech`; this surface must not become a secret viewer, env editor, restart/start/stop panel, token manager, approval bypass, raw SQL runner, restore runner, raw inventory viewer or remote-assist action launcher. Dangerous actions require separate control-plane hardening and must not be added as Tech Panel convenience buttons. | tech snapshot/locator tests, auth query-token counter tests, script marker tests, webapp tech tests/build, browser signoff for `/app/admin/tech` |
