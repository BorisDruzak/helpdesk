...
| Knowledge Platform | `server/knowledge/*`, `server/app/repos/knowledge_repo.py`, `server/web_api/knowledge_handlers.py`, `server/app/db/migrations/versions/*knowledge*`, `server/docs/KNOWLEDGE_PLATFORM.md`, `webapp/src/features/knowledge/*`, `webapp/src/pages/help/index.tsx`, `webapp/src/pages/knowledge/index.tsx`, `webapp/src/pages/admin/knowledge-page.tsx`, `pc_agent/ui_gui/server_api.py`, `pc_agent/ui_gui/chat_panel.py` | Universal knowledge spaces/items/versions/chunks, search/suggestions, ACL visibility, service/offering bindings, graph, ingestion, feedback/deflection metrics, passport-to-draft and support knowledge panel integration | `/api/knowledge/search|suggest|feedback`, `/api/web/knowledge/*`, selected-version publish, `knowledge_attempts` create payload extension, `ticket_knowledge_links`, existing `/api/tickets/{id}/kb_links` compatibility | knowledge contract/repo/visibility/search/suggestion/feedback/graph/ingestion/passport/API/metrics/publish/DB-constraint tests, P0/P1 regressions, webapp build, agent tests, browser signoff for `/app/admin/knowledge`, `/app/knowledge`, `/app/help`, `/app/tickets/:ticketId` |
...
| Knowledge Platform contract | `server/docs/KNOWLEDGE_PLATFORM.md`, `server/knowledge/*`, `server/app/repos/knowledge_repo.py`, `server/web_api/knowledge_handlers.py`, `server/app/db/models.py`, `webapp/src/features/knowledge/*`, `webapp/src/pages/help/index.tsx`, `pc_agent/ui_gui/server_api.py`, `pc_agent/ui_gui/chat_panel.py` | Admin/support/requester/agent knowledge UX, self-service deflection, support workspace suggestions, passport drafts, future retrieval/RAG | ACL must filter before direct reads/search/suggestions/graph/ingestion/metrics/retrieval; support/auditor must not see admin/security restricted items by default; requester/agent must not see internal/restricted content or raw source metadata; publish targets an explicit version; stale passport drafts require explicit review acknowledgement and never auto-publish; existing `kb_links` remains compatible; Protocol V3 unchanged unless explicitly redesigned |
