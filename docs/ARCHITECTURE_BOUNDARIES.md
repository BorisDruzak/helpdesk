...
| Server startup and routes | `server/server.py`, `server/routes.py`, `server/config.py`, `server/static_pages/*` | aiohttp app, route registration, startup/shutdown and React asset delivery | HTTP/WS route table, retired-entry redirects, runtime startup behavior | `python scripts/verify_workspace.py`, route/API tests, browser check for UI routes |
...
| External Registry boundary | `server/domain_ports/registry.py`, `server/domain_ports/registry_contracts.py`, `server/domain_ports/container.py`, `server/registry_adapter/local.py`, `server/registry_adapter/http.py`, `server/tickets/ticket_context.py`, `server/tickets/create_flow.py`, `server/inventory/service.py`, `server/web_api/requester_handlers.py`, `server/web_api/support_handlers.py`, `server/web_api/dto/support.py`, `server/tech/handlers.py`, `scripts/check_domain_import_boundaries.py`, `scripts/registry_retirement_manifest.py`, `scripts/rehearse_registry_retirement.py` | Versioned opaque refs, redacted requester/binding/account projections, purpose-bound ticket-participant and requester on-behalf projections/decisions, and bounded `InventoryQualityProjection`; local adapter is temporary compatibility composition, while HTTP/shadow reads preserve local authority; future table retirement is declared and read-only preflighted | Migrated Helpdesk consumers must use `RegistryPort`, correlate every outcome to the requested opaque ref, and never query local Registry as a hidden fallback. Ticket participant reads preserve only existing `ticket_context_v1` display/full/email and organisation refs and fail closed. Requester on-behalf operations require exact verified actor/requester/creator correlation plus a server-resolved form-policy snapshot; their adapter owns lifecycle/scope/exact-search decisions and generic directory search remains support/admin-only. The direct `PrimaryAgentResolver` remains explicit deferred diagnostic-target/response-decoration debt. Tech inventory quality preserves its numeric count, fails closed to zero on typed unavailable/invalid outcomes and exposes only redacted `source_state`; it never reads Registry entities directly. Support history may use only a validated immutable ticket snapshot. PR-11 additionally requires the absence of local runtime/imports, exact clone/count/FK/backup/restore linkage and trusted attestation. | scoped import-boundary/Registry boundary/requester/support/tech tests; ticket-builder contract tests; Registry HTTP/shadow tests; retirement-preflight/schema-audit tests; external shadow-read and command acceptance before Registry API cutover |
...
| Release, deploy and remote control | `scripts/release_server_to_remote.py`, `scripts/deploy_workspace_to_remote.py`, `scripts/manage_remote_stack.py`, `scripts/run_ci_suite.py`, `scripts/bootstrap_web_toolchain.py`, `docs/LOCAL_WORKFLOW.md`, `docs/WEBAPP_CUTOVER_CHECKLIST.md` | Verified release path, remote stack lifecycle, CI layering, React bundle, retired-entry redirect and browser signoff | Deploy script contract, artifact layout, remote server/control lifecycle, canonical ports | focused local checks by default; `python scripts/run_ci_suite.py --layer <name>` for targeted layers, full `python scripts/run_ci_suite.py` and full gate only by explicit user request, then remote smoke, browser signoff, stop server after checks |
