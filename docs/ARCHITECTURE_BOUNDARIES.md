...
| External Registry boundary | `server/domain_ports/registry.py`, `server/domain_ports/registry_contracts.py`, `server/domain_ports/container.py`, `server/registry_adapter/local.py`, `server/tickets/ticket_context.py`, `server/tickets/create_flow.py`, `server/inventory/service.py`, `server/web_api/support_handlers.py`, `server/web_api/dto/support.py`, `scripts/check_domain_import_boundaries.py` | Versioned opaque refs and redacted requester/binding/account projections; local adapter is temporary compatibility composition | Migrated Helpdesk consumers must use `RegistryPort`, correlate every outcome to the requested opaque ref, and never query local Registry as a hidden fallback. Support history may use only a validated immutable ticket snapshot. | scoped import-boundary/Registry boundary/requester/support tests; external shadow-read acceptance before Registry API cutover |
