...
| Service Catalog process layer | `server/tickets/service_catalog_*`, `server/app/repos/service_catalog_repo.py`, `server/web_api/service_catalog_handlers.py`, `webapp/src/features/service-catalog/*`, `webapp/src/pages/help/index.tsx`, `pc_agent/ui_gui/chat_panel.py`, `pc_agent/ui_gui/server_api.py` | Requester-facing services and offerings, publication gates, catalog-aware create/preview, service/offering policy inheritance and reporting dimensions | `/api/web/admin/service-catalog*`, `/api/service-catalog/*`, create payload fields `service_code`/`offering_code`, `custom_fields.service_catalog`, ticket reporting columns | service catalog tests, ticket create regression tests, webapp build, agent GUI helper/API tests, browser signoff for `/app/admin/service-catalog` and requester help flow |
