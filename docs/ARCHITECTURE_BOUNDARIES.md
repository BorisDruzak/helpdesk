...
| External Registry boundary | `server/domain_ports/registry.py`, `server/domain_ports/registry_contracts.py`, `server/domain_ports/container.py`, `server/registry_adapter/local.py`, `server/tickets/ticket_context.py`, `server/tickets/create_flow.py`, `server/inventory/service.py`, `server/web_api/support_handlers.py`, `server/web_api/dto/support.py`, `scripts/check_domain_import_boundaries.py`, `scripts/registry_retirement_manifest.py`, `scripts/rehearse_registry_retirement.py` | Versioned opaque refs and redacted requester/binding/account projections; local adapter is temporary compatibility composition; future table retirement is declared and read-only preflighted | Migrated Helpdesk consumers must use `RegistryPort`, correlate every outcome to the requested opaque ref, and never query local Registry as a hidden fallback. Support history may use only a validated immutable ticket snapshot. PR-11 additionally requires the absence of local runtime/imports, exact clone/count/FK/backup/restore linkage and trusted attestation. | scoped import-boundary/Registry boundary/requester/support tests; retirement-preflight/schema-audit tests; external shadow-read and command acceptance before Registry API cutover |
