...
| Knowledge Platform | `server/knowledge/*`, `server/app/repos/knowledge_repo.py`, `server/web_api/knowledge_handlers.py`, `server/app/db/migrations/versions/*knowledge*`, `server/docs/KNOWLEDGE_PLATFORM.md`, `server/docs/KNOWLEDGE_OPERATIONS.md`, `scripts/seed_knowledge_content.py`, `scripts/validate_knowledge_pack_bindings.py`, `scripts/repair_knowledge_pack_bindings.py`, `content_packs/knowledge/*`, `webapp/src/features/knowledge/*`, `webapp/src/pages/help/index.tsx`, `webapp/src/pages/knowledge/index.tsx`, `webapp/src/pages/admin/knowledge-page.tsx`, `webapp/src/pages/admin/knowledge-studio-page.tsx`, `pc_agent/ui_gui/server_api.py`, `pc_agent/ui_gui/chat_panel.py` | Universal knowledge spaces/items/versions/chunks, search/suggestions, ACL visibility, canonical Service Catalog content-pack bindings, binding repair, graph, ingestion, feedback/deflection metrics, passport-to-draft, content packs, templates/lint, review tasks, quality/gap operations, search analytics, rollout policies, article segmentation/index sync and support knowledge panel integration | `/api/knowledge/search|suggest|feedback`, `/api/web/knowledge/*`, selected-version publish, content-pack apply/retire, binding validator/repair commands, review-task/quality/gap/search-analytics/rollout APIs, effective rollout preview, segmentation/index-sync APIs, `knowledge_attempts` create payload extension, `ticket_knowledge_links`, existing `/api/tickets/{id}/kb_links` compatibility | knowledge contract/repo/visibility/search/suggestion/feedback/graph/ingestion/passport/API/metrics/publish/DB-constraint/content-pack/binding-repair/templates/lint/review/quality/gap/search-analytics/operations/segmentation tests, P0/P1 regressions, webapp build, agent tests, browser signoff for `/app/admin/knowledge`, `/app/admin/knowledge/studio`, `/app/knowledge`, `/app/help`, `/app/tickets/:ticketId` |
