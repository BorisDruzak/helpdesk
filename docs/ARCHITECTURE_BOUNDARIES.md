...
| Change Enablement contract | `server/docs/CHANGE_ENABLEMENT.md`, `server/change/*`, `server/web_api/change_handlers.py`, `server/app/db/models.py`, `webapp/src/features/changes/*`, `webapp/src/pages/admin/changes-page.tsx` | Support/admin change governance UX, P4 permanent-fix path, P3 improvement-action follow-up, service/offering change metrics | Change is not a ticket; normal/emergency changes require risk, plan and rollback before approval; approval records are first-class and auditable; blackout override requires justification; requester/public must not see change internals; P5 must not auto-execute technical changes or replace Problem Management/Improvement Actions |
