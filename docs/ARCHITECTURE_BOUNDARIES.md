...
| Tech Panel readiness surface | `server/tech/snapshot.py`, `server/tech/handlers.py`, `webapp/src/pages/admin/tech-page.tsx`, `webapp/src/features/tech/tech-panel-api.ts` | Read-only pilot readiness, security/auth/session/TLS policy visibility, runtime health, PostgreSQL/migrations/backup/restore markers, agents, operations, logs, release/smoke and safe links | `/api/web/admin/tech/snapshot` and `/app/admin/tech`; this surface must not become a secret viewer, env editor, restart/start/stop panel, token manager, approval bypass, raw SQL runner or remote-assist action launcher. Dangerous actions require separate control-plane hardening. | tech snapshot tests, webapp tech tests/build, browser signoff for `/app/admin/tech` |
