...
| Legacy web UI | `server/admin.*`, `server/support.*`, `server/ticket.*`, `server/help.*`, `server/web_shared.js` | Legacy admin/support/ticket/requester shells | Legacy endpoints, DOM contracts used by browser checks, shared JS helpers | browser check at `http://192.168.100.17:8666/admin`, relevant server tests |
