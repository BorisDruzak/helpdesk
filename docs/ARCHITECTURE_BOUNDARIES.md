...
| Service Catalog process layer | `server/tickets/service_catalog_*`, `server/app/repos/service_catalog_repo.py`, `server/web_api/service_catalog_handlers.py`, `scripts/seed_service_catalog.py`, `webapp/src/features/service-catalog/*`, `webapp/src/features/request-template-studio/*`, `webapp/src/pages/admin/request-template-studio-page.tsx`, `webapp/src/pages/help/index.tsx`, `pc_agent/ui_gui/chat_panel.py`, `pc_agent/ui_gui/server_api.py` | Requester-facing services and offerings, fallback `other.unknown`, publication gates, requester-safe preview, catalog-aware create/preview, service/offering policy inheritance and reporting dimensions. Request Template Studio is the primary no-code setup UX over existing Service Catalog, Forms Builder and Policy Health data: it edits a mutable Studio draft for form fields, title/description/visibility, processing profile, routing, SLA, approval, closure and notifications, then persists through existing form/catalog draft APIs. Service Catalog, Forms Builder and Policy Health remain source-of-truth expert surfaces, and direct Studio publish remains unavailable until a safe publish contract exists. | `/api/web/admin/service-catalog*`, `/api/web/admin/service-catalog/offerings/save-draft`, `/api/web/admin/forms/save-draft`, `/api/web/admin/helpdesk-model/*`, `/api/web/admin/helpdesk/policy-health*`, `/api/service-catalog/current`, `POST /api/service-catalog/preview`, create payload fields `service_code`/`offering_code`, `custom_fields.service_catalog`, ticket reporting columns | service catalog tests, ticket create regression tests, webapp build/test, agent GUI helper/API tests, browser signoff for `/app/admin/service-catalog`, `/app/admin/request-template-studio` and requester help flow |
