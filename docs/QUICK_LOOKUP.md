...
Context note: the current in-place refactor keeps the existing `modules` / `playbook_*` path, but the runtime tool contract is now wider than plain `module.tool`: server/agent resolve canonical semantic tool ids plus legacy aliases through the current registry/orchestrator path.
| Agent updates / recommended version | `server/agents/agent_builds_handlers.py`, `server/routes.py`, `pc_agent/ws_agent.py`, `pc_agent/ui_bridge/api_server.py`, `pc_agent/ui_gui/main_window.py` | `server/docs/AGENT_UPDATES_API.md`, `pc_agent/docs/AGENT_RUNTIME_ALWAYS_ON.md`, `pc_agent/docs/AGENT_UPDATE_WORKFLOW.md` |
| Modules / desired state / reconcile | `server/modules/handlers.py`, `server/tools/service.py`, `server/websocket/outbox_ingest_components.py`, `server/modules/reconcile.py`, `pc_agent/core/module_manager.py` | `server/docs/MODULES_API.md`, `server/docs/COMMAND_RESULT_LIFECYCLE.md`, `pc_agent/docs/MODULES.md` |
