...
- `/app/admin/inventory` now presents a low-noise agent console: metrics, table, right-side details, tabs for agents / connection requests / tokens / rollout, and the existing manual connection-request flow (`/api/admin/connection_policy`, `/api/admin/connection_requests*`) is available without opening legacy admin. The shell notification bell also counts pending connection requests alongside ticket notification unread count and links directly to `?panel=requests`.
