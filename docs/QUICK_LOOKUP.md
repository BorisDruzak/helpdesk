...
- `/app/admin/inventory` now has a device-token panel backed by `GET /api/web/admin/devices/{device_id}/tokens` and `POST /api/web/admin/devices/{device_id}/tokens/revoke`; tech alerts also include `inventory_env_uuid_duplicates` and `inventory_devices_without_location`.
