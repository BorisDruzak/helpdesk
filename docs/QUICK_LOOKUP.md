...
- Agent-side observer correlation for tech drilldown now uses `pc_agent/core/action_trace.py` plus the `search_action_trace` RPC in `pc_agent/ws_agent.py`; admin tech trace detail requests compact agent actions via `include_agent_actions=1`, the server bounds large `details` payloads before sync/render, the agent stores server `trace_id` in orchestrator/module action rows, and the agent now emits guaranteed module-level execution breakdown (`module.resolve`, `module.execute`) even when a tool module has no custom runtime audit hooks.
