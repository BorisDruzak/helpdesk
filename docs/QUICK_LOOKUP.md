...
- Request-template/effective registry `notification_policy` is executed in `server/tickets/notification_service.py`: per-event blocks such as `on_status_changed` and `on_sla_breach` select requester, assignee, queue and watcher recipients before user notification preferences apply. Backend smart views live in `server/tickets/smart_views.py` and are exposed through typed support queue `GET /api/web/support/queue?smart_view=...` with built-in options for `sla_risk`, `ola_risk`, `unassigned`, requester replies, stale waits, waiting approval and diagnostics failed; active published custom smart views from `smart_views` are appended to options and matched by their saved JSON filter.
