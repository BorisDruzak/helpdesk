...
- Published module live testing starts with `GET /api/modules/{module_name}/{version}/live_test_candidates?platform=win32|linux` so support/admin can choose the exact lab agent. A selected run creates `module_live_test` observer traces; blocked Windows preferred rollout creates `module_preferred_gate` and returns `observer_trace_id`. Module live tests are not ticket-bound; do not pass synthetic `ticket_id` values to the agent run, correlate them by `trace_id`/operation ids.
