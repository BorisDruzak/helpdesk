...
- Request forms can start diagnostics on ticket creation via `playbook_triggers`; `server/tickets/create_flow.py` builds a structured facts package from `request_form_data` / `request_form_summary`, starts the latest published playbook version idempotently, and `server/web_api/support_handlers.py` exposes `playbook_started` in the typed support detail timeline for `/app/tickets/:ticketId`.
