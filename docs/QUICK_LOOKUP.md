...
- 2026-04-29 priority/SLA slice: deterministic priority calculation lives in `server/tickets/priority_policy.py`. Form `priority_policy` maps submitted facts into `impact`, `urgency`, `importance`, `computed_priority`, `effective_priority` and `priority_decision`; `server/tickets/sla_service.py` resolves SLA targets by process priority P0..P3 with legacy P1..P4 fallback.
