...
- Support/tool UI preset selection now carries concrete preset params, while `server/web_api/support_handlers.py` re-expands presets before dispatch. `/app/admin/playbooks` shows module/install/platform/min-agent metadata and stores the selected tool manifest on each diagnostic block.
