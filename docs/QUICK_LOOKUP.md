...
- 2026-04-29 priority/SLA slice: deterministic priority calculation lives in `server/tickets/priority_policy.py`. Form `priority_policy` maps submitted facts into `impact`, `urgency`, `importance`, `computed_priority`, `effective_priority` and `priority_decision`; `server/tickets/sla_service.py` resolves SLA targets by process priority P0..P3 with legacy P1..P4 fallback. `server/tickets/form_catalog.py` preserves priority-policy fact keys from submitted payload even when they are agent-collected helper facts rather than visible form fields.
...
- Settings capabilities are intentionally split: `settings.view` opens the page, `settings.manage_queues` enables queue/member/OLA saves, and `settings.manage_routing` enables routing/SLA/calendar/resolution saves; denied actions should show the server-provided reason instead of falling back to one broad read-only mode.
