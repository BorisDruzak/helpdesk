...
- 2026-04-30 standalone helpdesk model registry starts in migrations `063`, `064` and `065`, `server/app/repos/helpdesk_policy_repo.py`, `server/tickets/helpdesk_policy_runtime.py`, `GET /api/web/admin/helpdesk-model/policies`, `POST /api/web/admin/helpdesk-model/request-templates/publish-from-form`, `POST /api/web/admin/helpdesk-model/policies/publish`, `POST /api/web/admin/helpdesk-model/policies/diff`, `POST /api/web/admin/helpdesk-model/policies/deactivate`, `POST /api/web/admin/helpdesk-model/policies/rollback` and `POST /api/web/admin/helpdesk-model/smart-views/publish`: the forms builder can publish the selected visual template into separate versioned `request_templates`, priority/SLA/OLA/routing/approval/closure/diagnostic/visibility/notification/reporting policy tables, direct policy editors can publish all those policy versions plus smart views, compare policy JSON versions, soft-deactivate policy versions and rollback by publishing a new active version, audit rows are written for publication/deactivate/rollback, ticket creation overlays effective registry policies as `system -> ticket_type -> category -> request_template`, existing-ticket lifecycle runtime also resolves effective approval/closure/notification/visibility/reporting policies without mutating historical ticket snapshots, active custom smart views are executable in `GET /api/web/support/queue?smart_view=...`, standalone `sla_policies.config.targets` can calculate first-response/resolution due dates without a legacy `ticket_sla_policies` id, and reporting policies govern passport sections, evidence/action packages, export visibility and report tags.
...
- 2026-04-30 configurable workflow slice: workflow profiles are stored in `ServerConfig` key `ticket.workflow_profiles` with code defaults as fallback. `PUT /api/web/settings/workflow_profiles` saves process type labels, required fields, flags, suggested path and status transition maps; structured transition entries can define `allowed_roles`, `required_fields`, `required_comment`, `require_approval`, `require_evidence` and `actions.notify`/`actions.sla`, which `server/tickets/workflow_service.py` enforces or records on status changes for the ticket's `ticket_type`. `/app/settings` edits these profiles through a visual transition rule constructor while preserving JSON compatibility, and `/app/admin/forms` reads the configured profile list for request-template `ticket_type` selection.
...
| Ticket flows / helpdesk | `server/tickets/handlers.py`, `server/tickets/create_flow.py`, `server/tickets/workflow_service.py`, `server/tickets/routing_service.py`, `server/tickets/form_catalog.py`, `server/tickets/helpdesk_policy_runtime.py`, `server/tickets/notification_service.py`, `server/tickets/visibility_policy.py`, `server/tickets/smart_views.py`, `server/tickets/passport_service.py`, `server/app/repos/ticket_passport_repo.py`, `server/web_api/support_handlers.py`, `server/web_api/settings_handlers.py` | `server/docs/TICKET_SYSTEM.md`, `server/docs/CODEMAP.md`, `server/docs/REQUEST_FORM_BUILDER.md`; паспорт решения хранит versioned official dossier, evidence/action/approval/related-object facts and powers `/app/tickets/:ticketId`; request-template catalog is exposed as `ticket_settings.request_templates`, and effective registry approval/closure/notification/visibility policies plus typed support smart views are backend-enforced before visual changes |
