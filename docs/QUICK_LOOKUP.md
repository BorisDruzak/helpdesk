...
| New web workspaces / typed web boundary | `webapp/src/main.tsx`, `webapp/src/app/router.tsx`, `webapp/src/app/layouts/app-shell.tsx`, `webapp/src/features/auth/session-provider.tsx`, `webapp/src/features/auth/login-page.tsx`, `webapp/src/features/auth/workspace-access.ts`, `webapp/src/shared/realtime/adapters/ws-ui-bridge.ts`, `webapp/src/shared/realtime/client.ts`, `webapp/src/features/queues/api.ts`, `webapp/src/features/queues/support-workspace.tsx`, `webapp/src/features/admin/api.ts`, `webapp/src/features/admin/admin-workspace.tsx`, `webapp/src/features/agent-updates/api.ts`, `webapp/src/features/agent-updates/device-update-panel.tsx`, `webapp/src/features/modules/api.ts`, `webapp/src/features/modules/modules-panel.tsx`, `webapp/src/features/forms-builder/api.ts`, `webapp/src/features/forms-builder/forms-builder-panel.tsx`, `webapp/src/features/tech/api.ts`, `webapp/src/features/tech/observer-quick-panel.tsx`, `webapp/src/features/tech/observer-trace-drilldown.tsx`, `webapp/src/pages/admin/index.tsx`, `webapp/playwright.config.ts`, `webapp/scripts/remote-browser-signoff.mjs`, `webapp/package.json`, `webapp/tests/support-workspace.spec.ts`, `webapp/tests/admin-workspace.spec.ts`, `webapp/tests/fixtures/support_fixture_server.py`, `server/web_api/session_handlers.py`, `server/web_api/support_handlers.py`, `server/web_api/admin_handlers.py`, `server/web_api/realtime_handlers.py`, `server/static_pages/webapp_assets.py`, `server/static_pages/handlers.py`, `server/config.py`, `server/routes.py`, `server/websocket/ui_handler.py` | `docs/superpowers/specs/2026-04-20-admin-support-web-rearchitecture-design.md`, `docs/superpowers/plans/2026-04-20-admin-support-web-rearchitecture.md`, `server/docs/SECURITY_AND_AUTH.md`, `server/docs/OBSERVER_LAYER.md`, `server/docs/REQUEST_FORM_BUILDER.md`; support wave now also includes `GET /api/web/support/tickets/{ticket_id}/tools`, `POST /api/web/support/tickets/{ticket_id}/tools/run`, tool events in the typed timeline и live realtime invalidation через `GET /api/web/realtime/bootstrap` -> `/ws_ui`, но support queue больше не подписывает realtime на всю видимую выборку: очередь держится на polling, а websocket остаётся только у активного тикета, чтобы вход в `/app/support` не создавал сотни `subscribe_ticket`; legacy admin fallback queue в `server/admin.js` теперь тоже оставлена на polling-only и больше не шлёт массовые `subscribe_ticket` на весь видимый список, если кто-то заходит в `?legacy=1`; admin wave 1 теперь кроме `GET /api/web/admin/devices` добавляет typed device update surface `GET /api/web/admin/devices/{device_id}/updates` + `POST /api/web/admin/devices/{device_id}/updates/run`, device-scoped observer slices `GET /api/web/admin/observer/quick`, `GET /api/web/admin/observer/traces` и `GET /api/web/admin/observer/traces/{trace_id}`, typed modules registry/actions `GET /api/web/admin/modules`, `PATCH /api/web/admin/modules/rollout_settings`, `PATCH /api/web/admin/modules/{module_name}/preferred`, новый forms-builder slice `GET /api/web/admin/forms/current` + `POST /api/web/admin/forms/save`, а также общий typed realtime bridge для live refresh карточек тикетов и device detail в `/app/admin`; `GET /api/web/session/me` теперь дополнительно рекламирует `default_workspace` и `available_workspaces`, `/app` выбирает рабочую область по роли, а legacy `/login`, `/support`, `/admin` по умолчанию уже ведут в новый shell, сохраняя явный fallback `?legacy=1`; canonical live signoff после релиза теперь идёт через `pnpm --dir webapp run check:remote:webapp -- --base-url http://192.168.100.17:8666`, который подтверждает `/app`, `/app/admin`, `/app/support`, session payload, русскую локаль, deep admin panels и отсутствие browser console/page errors |
