...
- 2026-04-30 standalone helpdesk model registry starts in migration `063`, `server/app/repos/helpdesk_policy_repo.py`, `GET /api/web/admin/helpdesk-model/policies` and `POST /api/web/admin/helpdesk-model/request-templates/publish-from-form`: the forms builder can publish the selected visual template into separate versioned `request_templates`, priority/routing/approval/closure/diagnostic/visibility/notification policy tables, smart-view storage and audit rows, while inheritance is resolved as `system -> ticket_type -> category -> request_template`.
