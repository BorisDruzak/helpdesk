...
For live validation, debugging, browser/admin UI, Endpoint operation lifecycle or deployment/runtime-control work, also follow `docs/LIVE_TESTING_DEBUG_RULES.md`. That document is the stricter source for evidence, validation surfaces and milestone final gates.
...
python -m pytest server/tests/test_ticket_*.py server/tests/test_helpdesk_*.py -m "not manual and not no_db" -vv --durations=80
python -m pytest server/tests/test_migration_schema_contract.py -m "not manual and not no_db" -vv --durations=80
```
...
- DB/API domain layers are split into tickets/helpdesk, observer/diagnostics, operations/runtime compatibility, and web/API catch-all layers by the canonical `quality/test_suites.toml` catalog. `scripts/run_ci_suite.py` and `scripts/audit_test_inventory.py` load the same catalog for filename ownership, affected-suite source-prefix routing, layer order and DB parallel grouping.
...
parallelism with two workers. For a direct server DB runtime measurement or an explicit CI-speed check, use the
same bounded mode instead of `pytest-xdist`:

```powershell
python scripts/run_ci_suite.py --parallel --max-workers 2
python scripts/run_ci_suite.py --parallel --max-workers 2 --layer server_pytest_db_agent_runtime --layer server_pytest_db_observer_diagnostics
python scripts/run_ci_suite.py --parallel --parallel-measurements artifacts/ci/<sha>/fixture-timings-summary.json
```

`--parallel` only groups independent server DB pytest layers. The runner keeps `verify_workspace`,
webapp build/unit/e2e, and `server_pytest_no_db` sequential before the group. A single requested DB layer remains sequential. Start with `--max-workers 2`; values above `3`
are allowed but risky until the remote PostgreSQL and SSH tunnel behavior is proven stable.

On Windows, parallel DB mode is guarded against child pytest processes owning a shared DB tunnel. If
...
`fixture-timings-summary.json`; if its `budget_status` is `fail`, the effective worker count is capped at 1 and DB
...
- 8 hours per DB-backed server pytest layer. Measured parallel isolated staging PostgreSQL profiles can exceed the former four-hour limit; the separate idle timeout and fixture-timing budget gates remain active. Audits, mutation smoke, script pytest and the no-DB server layer retain their 45-minute fail-fast ceiling.
- The configured idle timeout for all CI steps.
...
- `PC_CLIENT_TEST_DB_TEMPLATE=1` for DB server pytest layers, so isolated layer databases are cloned from a migrated PostgreSQL template keyed by the Alembic migration fingerprint. The `migration_schema` layer is the exception and forces `PC_CLIENT_TEST_DB_TEMPLATE=0` to prove the direct empty-DB Alembic path.
...
- `db_cleanup_profile_audit` runs `python scripts/audit_db_cleanup_profiles.py --strict` before pytest layers and fails on DB-backed server test files without an explicit `db_cleanup` profile.
...
- Optional `--parallel --max-workers 2` for bounded server DB layer concurrency; this does not change pytest markers,
...
python -m pytest server/tests/test_web_support_api.py -m "not manual and not no_db" -q
...
Do not use light fixtures for files that require live WebSocket transport or broad server-side side effects. Keep those tests on the regular `test_client` unless a separate PR proves and documents a safe noop substitute.
...
`PC_CLIENT_TEST_DB_TEMPLATE=1` is an opt-in server pytest acceleration path for PostgreSQL DB layers. The harness computes a stable fingerprint from `server/alembic.ini` and `server/app/db/migrations/**`, creates or reuses `pc_support_test_template_<fingerprint12>`, applies `alembic upgrade head` once to that template, then creates each isolated `pc_support_test_<domain>_<worker>_<hash>` database with `CREATE DATABASE ... TEMPLATE ...`.

This removes repeated Alembic setup for DB layers, but it does not change `test_app`, `test_client`, pooling, xdist, or test semantics. Tests without an explicit cleanup profile still pay the full per-test `cleanup_db` `TRUNCATE ... RESTART IDENTITY CASCADE` cost.
...
$env:PC_CLIENT_TEST_TIMING = "1"
$env:PC_CLIENT_TEST_CLEANUP_AUDIT = "1"
python -m pytest server/tests/test_ticket_*.py server/tests/test_helpdesk_*.py -m "not manual and not no_db" -vv --durations=40
```
...
- Browser `/ws_ui` checks remain in the browser/no-DB workflow; endpoint operation facades belong to their focused DB/API layer.
- Endpoint agent runtime changes are validated in the Endpoint Platform repository; Helpdesk validates only its Endpoint facade and real-agent acceptance evidence.
