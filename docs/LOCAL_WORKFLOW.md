...
7. При необходимости отправить commit на Linux через Git:

```powershell
git push linux master
```

8. При необходимости посмотреть, что уйдёт на шару:

```powershell
python scripts/sync_local_to_share.py
```

9. Применить синхронизацию на шару:

```powershell
python scripts/sync_local_to_share.py --apply
```

10. Выложить локальную копию на Linux-хост:

```powershell
python scripts/deploy_workspace_to_remote.py
```

11. Поднять сервер на Linux и прогнать smoke:
...
13. После проверок остановить процессы:
