    ...
    action_queues: {
      no_audience_users: {
        ...
        items: [
          {
            item_id: "item-zero",
            title: "VPN для заявителей",
            reason: "Аудитория не содержит пользователей",
            action_url: "/app/admin/knowledge/studio?item=item-zero",
          },
        ],
      },
      missing_helpdesk_binding: {
        ...
        items: [
          {
            item_id: "item-binding",
            title: "Принтеры",
            reason: "Нет связи с услугой или формой обращения",
            action_url: "/app/admin/knowledge/studio?item=item-binding",
          },
        ],
      },
      stale_article: {
        ...
        items: [
          {
            ...
            action_url: "/app/admin/knowledge/studio?item=item-stale",
          },
        ],
      },
      indexing_failed: {
        ...
        items: [
          {
            ...
            action_url: "/app/admin/knowledge/studio?item=item-quality",
          },
        ],
      },
      zero_result_searches: {
        ...
        items: [
          {
            query: "vpn",
            reason: "5 поисков без результата",
...
function mockKnowledgeFetch() {
  const fetchMock = vi.fn((url: string) => {
    ...
    if (url === "/api/web/knowledge/metadata") {
      return Promise.resolve(jsonResponse(metadataPayload));
    }
...
describe("KnowledgeOpsDashboardPanel", () => {
  it("renders the operations dashboard with Russian queue labels and no mojibake", async () => {
    const fetchMock = mockKnowledgeFetch();
    ...
    expect(screen.getAllByText("Ошибки индексации").length).toBeGreaterThanOrEqual(1);
    expect(screen.getByRole("heading", { name: "Очереди действий" })).toBeInTheDocument();
    expect(screen.getByText("Статьи без аудитории")).toBeInTheDocument();
    expect(screen.getByText("Статьи без связи с обращениями")).toBeInTheDocument();
    expect(screen.getByText("Просроченные статьи")).toBeInTheDocument();
    expect(screen.getByText("Низкое качество")).toBeInTheDocument();
    expect(screen.getByText("Поиски без результата")).toBeInTheDocument();
    ...
    expect(container.textContent ?? "").not.toMatch(/raw JSON|Pack JSON|Content packs|content pack/i);
    expect(container.textContent ?? "").not.toMatch(/\uFFFD|Рђ|РЎ|Рџ|Рќ|Р°|Рµ|СЃ|С‚|С‹|С‡/);
  ...
  it("shows route-specific next actions for every queue item", async () => {
    mockKnowledgeFetch();
    const { container } = renderPanel();

    await screen.findByText("Очереди действий");

    expect(screen.getByRole("link", { name: "Проверить аудиторию" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/studio?item=item-zero",
    );
    expect(screen.getByRole("link", { name: "Настроить связь" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/studio?item=item-binding",
    );
    expect(screen.getByRole("link", { name: "Обновить статью" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/studio?item=item-stale",
    );
    expect(screen.getByRole("link", { name: "Открыть индексацию" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/indexing",
    );
    expect(screen.getByRole("link", { name: "Улучшить качество" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/studio?item=item-quality",
    );
    expect(screen.getByRole("link", { name: "Проверить поиск" })).toHaveAttribute(
      "href",
      "/app/admin/knowledge/search-settings",
    );
    const createArticleLink = screen.getByRole("link", { name: "Создать статью" });
    expect(createArticleLink).toHaveAttribute(
      "href",
      "/app/admin/knowledge/import",
    );
    expect(createArticleLink).toHaveAttribute("title", "Открыть импорт или создание черновика для запроса без результата.");
    expect(screen.getByRole("link", { name: "Разделы" })).toHaveAttribute("href", "/app/admin/knowledge/sections");
    expect(container.textContent ?? "").not.toMatch(/target_id|subject_id|metadata_json|rule_id|binding_id|job_id/);
  });

  it("uses active metadata counts and keeps metadata diagnostics product-facing", async () => {
    mockKnowledgeFetch();
    renderPanel();
