...
export type StudioPolicyKind = (typeof POLICY_KINDS)[number];
...
export function serviceOptions(services: AdminServiceCatalogService[]): PickerOption[] {
  return services.map((service) => ({
    ...
    subtitle: [
      ...
      service.visibility,
      service.owner_queue_id ? `Очередь ${service.owner_queue_id}` : null,
    ]
      .filter(Boolean)
      .join(" · "),
...
export function offeringOptions(
  ...
  return offerings
    ...
    .map((offering) => ({
      ...
      subtitle: [
        offering.request_template_key ? `Шаблон ${offering.request_template_key}` : "Шаблон не привязан",
        offering.visibility,
      ]
        .filter(Boolean)
        .join(" · "),
...
export function requestTemplateOptions(registry?: AdminHelpdeskModelPayload | null): PickerOption[] {
  return (registry?.request_templates ?? []).map((template) => ({
    ...
    disabledReason: template.is_active ? null : "Шаблон неактивен",
  }));
}
...
export function policyOptions(
  ...
  return (registry?.policies?.[kind] ?? []).map((policy) => ({
    ...
    disabledReason: policy.is_active ? null : "Политика неактивна",
  }));
}
...
export function buildStudioSimulationPayload({
  ...
  // Catalog codes stay in custom_fields only for legacy previews; Policy Health uses the top-level fields as canonical context.
