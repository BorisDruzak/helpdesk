...
describe("appRoutes", () => {
  ...
  it("does not register local Knowledge pages", () => {
    const paths = collectRoutePaths(appRoutes);

    expect(paths).not.toContain("kb");
    expect(paths).not.toContain("kb/articles/:slug");
    expect(paths).not.toContain("kb/spaces/:spaceCode");
    expect(paths).not.toContain("kb/tags/:tag");
    expect(paths).not.toContain("kb/search");
    expect(paths).not.toContain("kb/ask");
    expect(paths).not.toContain("knowledge");
    expect(paths).not.toContain("knowledge/articles/:itemId");
    expect(paths.some((path) => path === "admin/knowledge" || path.startsWith("admin/knowledge/"))).toBe(false);
  });
